...
def _create_event(lead, *, from_stage="", to_stage="", event_type="transition", occurred_at=None, actor=None, metadata=None):
...
def record_initial_funnel_event(lead, *, actor=None, occurred_at=None, backfilled=False, metadata=None):
...
def record_status_funnel_event(lead, *, from_status, to_status, actor=None, occurred_at=None, backfilled=False, metadata=None):
    if not to_status:
        return None

    from_stage = funnel_stage_for_lead_status(from_status) if from_status else ""
    to_stage = funnel_stage_for_lead_status(to_status)
    event_metadata = {"from_status": from_status or "", "to_status": to_status, "backfilled": backfilled}
    event_metadata.update(metadata or {})
    ...
    if not to_stage:
        return None
    if from_stage == to_stage and LeadFunnelEvent.objects.filter(lead=lead, to_stage=to_stage).exists():
        return None
...
def backfill_lead_funnel_events(queryset=None):
    ...
    for lead in leads.prefetch_related("activities", "funnel_events"):
        if not lead.funnel_events.filter(event_type="initial").exists():
            record_initial_funnel_event(lead, occurred_at=lead.created_at, backfilled=True)
            created += 1
        ...
        if current_stage and not lead.funnel_events.filter(to_stage=current_stage).exists():
            event = record_status_funnel_event(
                lead,
                from_status="",
