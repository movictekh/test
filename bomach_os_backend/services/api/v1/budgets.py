...
def create_budget(request, data: BudgetIn):
    ...
    try:
        if Budget.objects.filter(
            ...
            return 400, {
                "detail": "A budget already exists for this branch, department, and period."
            }
...
@router.put(
    "/{budget_id}", response={200: BudgetOut, 400: MessageSchema, 404: MessageSchema}
)
...
@router.delete(
    "/{budget_id}", response={204: None, 400: MessageSchema, 404: MessageSchema}
)
