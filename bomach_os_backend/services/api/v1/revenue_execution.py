...
from user.models.role_targets import EmployeeTarget, RoleTargetTemplate, with_target_progress
from user.utils.perm import require_permission, scope_queryset


router = Router(tags=["Revenue Execution"])
...
MANAGEMENT_RHYTHM = [
    {"time": "9:00 AM", "name": "Revenue huddle", "focus": "Red metrics, top deals, blockers and commitments"},
    {"time": "1:00 PM", "name": "Pipeline control", "focus": "SLA breaches, next actions and campaign quality"},
    {"time": "5:00 PM", "name": "Close-out", "focus": "Results, misses, learning and tomorrow’s first actions"},
    {"time": "Friday 4 PM", "name": "Executive review", "focus": "Forecast, ROI, people decisions and resource shifts"},
]
...
LEAD_SCORING_MODEL = [
    {"points": 40, "name": "Customer fit", "copy": "Division match, location, budget, authority and service suitability."},
    {"points": 30, "name": "Purchase intent", "copy": "Inspection request, proposal request, payment question or decision deadline."},
    {"points": 20, "name": "Engagement", "copy": "Replies, calls, brochure views, event attendance and repeat visits."},
    {"points": 10, "name": "Timing", "copy": "Ready now, within 30 days, 90 days, or long-term nurture."},
]
...
def _day_queryset(request):
    qs = DailyExecutionDay.objects.select_related("branch", "opened_by").prefetch_related(
        "actions",
        "actions__owner",
...
def _lead_queryset(request):
    qs = Lead.objects.select_related("assigned_to", "assigned_to__user", "assigned_to__role", "branch")
    return scope_queryset(request, qs, branch_field="branch_id")
...
def _playbook_queryset(request):
    qs = SalesPlaybook.objects.select_related("branch", "created_by").prefetch_related("objections")
...
def _playbook_objection_queryset(request):
    ...
    branch_ids = getattr(request, "_perm_branch_ids", [])
    if branch_ids:
        return qs.filter(Q(playbook__branch_id__in=branch_ids) | Q(playbook__branch__isnull=True))
    return qs
...
def _decimal_sum(queryset, field_name):
    ...
    return total.quantize(Decimal("0.01")) if isinstance(total, Decimal) else Decimal(total).quantize(Decimal("0.01"))
...
def _decimal_pct(numerator, denominator):
    ...
    return min((numerator / denominator) * Decimal("100.00"), Decimal("100.00")).quantize(Decimal("0.01"))
...
def _money_display(value):
    ...
    if value >= Decimal("1000000"):
        ...
        display = millions.quantize(Decimal("0.1")) if value % Decimal("1000000") else millions.quantize(Decimal("1"))
        return f"₦{display}M"
...
def _scenario_factor(scenario):
    return FORECAST_SCENARIOS.get(scenario or "base", FORECAST_SCENARIOS["base"])["factor"]
...
def _weighted_forecast_value(leads, factor=Decimal("1.00")):
    ...
    for row in leads.values("status").annotate(total=Sum("estimated_value")):
        weighted += (row["total"] or Decimal("0.00")) * LEAD_STATUS_FORECAST_WEIGHTS.get(
            row["status"],
            Decimal("0.00"),
        )
...
def _forecast_quality_controls(active_leads, now=None):
    ...
    next_action_pct = _decimal_pct(
        active_leads.exclude(next_action="").filter(next_follow_up_at__isnull=False).count(),
        total,
    )
    stage_age_pct = _decimal_pct(
        active_leads.filter(updated_at__gte=now - timedelta(days=FORECAST_STAGE_AGE_LIMIT_DAYS)).count(),
        total,
    )
...
def _forecast_payload(request, period_start=None, period_end=None, branch_id=None, division=None, scenario="base"):
...
def _lead_sla_status(lead, now=None):
    ...
    if not due_at and lead.created_at:
        due_at = lead.created_at + timezone.timedelta(minutes=Lead.DEFAULT_FIRST_RESPONSE_MINUTES)
...
def _lead_control_rows(leads, now, limit):
    ...
    for lead in leads[:limit]:
        ...
        rows.append({
            ...
            "age_days": age_days,
            "next_action": lead.next_action or _recommended_action(lead, sla_status),
            "sla_status": sla_status,
            "sla_label": "Breach" if sla_status == "breached" else "Due now" if sla_status == "due_now" else "Safe",
            "owner": _employee_name(lead.assigned_to) or "Unassigned",
            "actions": [
                {"label": "Open", "action": "open"},
                {"label": "Contact", "action": "contact"} if sla_status in ["breached", "due_now"] or lead.status == "new" else None,
            ],
        })
    ...
    return [
        {
            ...
            "value": len([
                lead for lead in active_leads
                if lead.status == "new" and not lead.first_contact_at and not lead.first_response_at
            ]),
        ...
        },
        {
            "key": "sla_breaches",
            "label": "SLA breaches",
            "value": len([lead for lead in active_leads if _lead_sla_status(lead, now) == "breached"]),
...
def _eligible_revenue_employees(request, branch_id=None):
    ...
    employees = Employee.objects.select_related("user", "role", "branch").filter(
        is_active=True,
...
def _stage_lead_sets(events, cohort_ids):
    return {
        stage: set(
            events.filter(lead_id__in=cohort_ids, to_stage=stage).values_list("lead_id", flat=True).distinct()
        )
...
def _lead_value_sum(leads, lead_ids):
    return leads.filter(id__in=lead_ids).aggregate(total=Sum("estimated_value"))["total"] or Decimal("0.00")
...
def _transition_leaks(stage_sets, leads):
    ...
    return sorted(leaks, key=lambda row: (row["loss_pct"], row["revenue_impact"]), reverse=True)
...
def _phase_summary(plan):
    ...
    for phase, title, period in TURNAROUND_PHASES:
        ...
        completed = len([action for action in phase_actions if action.status == "completed"])
        grouped.append({
            "phase": phase,
            "title": title,
            "period": period,
...
def get_funnel_audit(
    ...
    division_conversion = []
    for division_key, division_label in Lead.DIVISION_CHOICES:
        division_lead_ids = set(cohort_leads.filter(division=division_key).values_list("id", flat=True))
        if not division_lead_ids:
            continue
        division_stage_sets = _stage_lead_sets(events, division_lead_ids)
        division_leaks = _transition_leaks(division_stage_sets, cohort_leads.filter(division=division_key))
        ...
        division_conversion.append({
            ...
            "biggest_leak": division_leaks[0]["transition"] if division_leaks else None,
        })
...
@require_permission("revenue_execution", "view")
def get_command_center(request, date: date = None, period_start: date = None, period_end: date = None, branch_id: int = None):
    ...
    active_leads = leads.filter(status__in=Lead.ACTIVE_STATUSES)
    won_leads = leads.filter(status="won", updated_at__date__gte=start, updated_at__date__lte=end)
    ...
    total_actions, completed_actions, open_actions, completion_pct = _completion_counts(day) if day else (0, 0, 0, 0)
    active_count = active_leads.count()
    leads_with_next_action = active_leads.exclude(next_action="").filter(next_follow_up_at__isnull=False).count()
    follow_up_compliance = round((leads_with_next_action / active_count) * 100, 2) if active_count else 0.0
    sla_breaches = sum(1 for lead in active_leads if _lead_sla_status(lead, now) == "breached")
    ...
    priorities = []
    if day:
        for action in day.actions.select_related("owner", "owner__user").order_by("sort_order", "id")[:5]:
            priorities.append({
                "id": action.id,
                "title": action.title,
                "meta": action.description or f"{_employee_name(action.owner) or 'Unassigned'} · Daily revenue action",
                "severity": action.severity,
                "done": action.status == "completed",
            })

    qualified_count = period_leads.filter(status__in=["qualified", "proposal_sent", "negotiation", "won"]).count()
    intent_count = period_leads.filter(status__in=["proposal_sent", "negotiation"]).count()
    won_count = period_leads.filter(status="won").count()
    funnel = [
        {"stage": "awareness", "name": "Awareness", "number": period_leads.count(), "rate_label": "Reach", "drop_label": "Top of funnel"},
        {"stage": "discovery", "name": "Discovery", "number": period_leads.count(), "rate_label": "Leads", "drop_label": "Lead capture"},
        {"stage": "evaluation", "name": "Evaluation", "number": qualified_count, "rate_label": "Qualified", "drop_label": f"{round((qualified_count / period_leads.count()) * 100, 1) if period_leads.count() else 0}% from prior"},
        {"stage": "intent", "name": "Intent", "number": intent_count, "rate_label": "Meetings", "drop_label": f"{round((intent_count / qualified_count) * 100, 1) if qualified_count else 0}% from prior"},
        {"stage": "purchase", "name": "Purchase", "number": won_count, "rate_label": "Won", "drop_label": f"{round((won_count / intent_count) * 100, 1) if intent_count else 0}% from prior"},
        {"stage": "loyalty", "name": "Loyalty", "number": 0, "rate_label": "Repeat/referral", "drop_label": "Not tracked in this slice"},
    ]

    team_snapshot = []
    for employee_id in active_leads.exclude(assigned_to__isnull=True).values_list("assigned_to_id", flat=True).distinct():
        ...
        owned_with_next_action = employee_leads.exclude(next_action="").filter(next_follow_up_at__isnull=False).count()
        score = round((owned_with_next_action / total) * 100) if total else 0
        team_snapshot.append({
            "role": _role_label(employee),
            "score": score,
            "revenue_indicator": "On pace" if score >= 80 else "At risk" if score >= 60 else "Needs action",
            "priority_coaching": "Pipeline discipline" if score >= 80 else "Follow-up cadence",
        })
    ...
    return {
        ...
        "hero": {
            ...
            "status": "on_track" if health_score >= 80 else "at_risk" if health_score >= 50 else "off_track",
        ...
        "kpi_cards": [
            {"key": "revenue_closed", "label": "Revenue closed", "value": revenue_closed, "display_value": _money_display(revenue_closed), "foot": "Target pace: ₦37.5M per month", "icon": "ti-currency-naira", "bg": "#D1FAE5", "color": "#065F46"},
            {"key": "weighted_forecast", "label": "Weighted forecast", "value": weighted_forecast, "display_value": _money_display(weighted_forecast), "foot": f"{_decimal_pct(weighted_forecast, ninety_day_target)}% of 90-day target", "icon": "ti-chart-arrows-vertical", "bg": "#DBEAFE", "color": "#1E40AF"},
            {"key": "qualified_pipeline", "label": "Qualified pipeline", "value": qualified_count, "display_value": qualified_count, "foot": "Deals with verified need and timing", "icon": "ti-filter-check", "bg": "#EDE9FE", "color": "#5B21B6"},
            {"key": "follow_up_compliance", "label": "Follow-up compliance", "value": follow_up_compliance, "display_value": f"{follow_up_compliance}%", "foot": f"{sla_breaches} SLA breaches require action", "icon": "ti-clock-check", "bg": "#FEF3C7", "color": "#92400E"},
            {"key": "daily_execution", "label": "Daily execution", "value": completion_pct, "display_value": f"{completion_pct}%", "foot": f"{completed_actions} of {total_actions} non-negotiables complete", "icon": "ti-bolt", "bg": "#FCE7F3", "color": "#9D174D"},
        ],
...
def export_forecast(
    ...
    response["Content-Disposition"] = 'attachment; filename="bomach-revenue-forecast.csv"'
    return response
...
def get_lead_control(
    ...
    if normalized_filter == "breach":
        ids = [lead.id for lead in base_leads.filter(status__in=Lead.ACTIVE_STATUSES) if _lead_sla_status(lead, now) == "breached"]
        ...
        ids = [lead.id for lead in base_leads.filter(status__in=Lead.ACTIVE_STATUSES) if lead.is_stale]
        filtered = base_leads.filter(id__in=ids)
    elif normalized_filter == "reactivate":
        filtered = base_leads.filter(Q(status="lost") | Q(status="contacted") | Q(status="dormant"))
...
@router.post("/lead-control/auto-assign")
...
def auto_assign_leads(request, branch_id: int = None, limit: int = 250):
    ...
    if branch_id:
        leads = leads.filter(branch_id=branch_id)
    leads = list(leads.order_by("branch_id", "created_at", "id")[:min(max(limit, 1), 500)])
    assigned_count = 0

    eligible_by_branch = {}
    company_pool = list(_eligible_revenue_employees(request, branch_id=None).filter(branch__isnull=True))
    for lead in leads:
        branch_key = lead.branch_id or "company"
        if branch_key not in eligible_by_branch:
            branch_pool = list(_eligible_revenue_employees(request, branch_id=lead.branch_id)) if lead.branch_id else company_pool
    ...
    return {"assigned_count": assigned_count, "skipped_count": len(leads) - assigned_count}


@router.post("/lead-control/repair-next-actions")
@require_permission("revenue_execution", "update")
def repair_next_actions(request, branch_id: int = None, limit: int = 500):
    leads = _lead_queryset(request).filter(status__in=Lead.ACTIVE_STATUSES).filter(
        Q(next_action="") | Q(next_follow_up_at__isnull=True)
    )
    if branch_id:
        leads = leads.filter(branch_id=branch_id)
    leads = list(leads.order_by("-score", "created_at", "id")[:min(max(limit, 1), 1000)])
...
@router.get("/playbooks/current", response={200: dict, 404: MessageSchema})
@require_permission("revenue_execution", "view")
def get_current_sales_playbook(
    ...
    playbook = None
    if branch_id:
        playbook = playbooks.filter(branch_id=branch_id).order_by("sort_order", "title", "id").first()
    if not playbook:
        playbook = playbooks.filter(branch__isnull=True).order_by("sort_order", "title", "id").first()
    if not playbook:
        return 404, {"detail": "No active sales playbook found for this division, stage and persona."}
...
def list_sales_playbooks(
    ...
    return {
        ...
        "playbooks": [_playbook_row(playbook) for playbook in playbooks.order_by("sort_order", "title", "id")[:limit]],
    }


@router.post("/playbooks", response={201: dict, 400: MessageSchema})
...
@router.get("/playbooks/{playbook_id}", response={200: dict, 404: MessageSchema})
@require_permission("revenue_execution", "view")
...
@router.patch("/playbooks/{playbook_id}", response={200: dict, 400: MessageSchema, 404: MessageSchema})
@require_permission("revenue_execution", "update")
def update_sales_playbook(request, playbook_id: int, payload: SalesPlaybookUpdateSchema):
    try:
        playbook = get_object_or_404(_playbook_queryset(request), id=playbook_id)
...
@router.delete("/playbooks/{playbook_id}", response={200: MessageSchema, 404: MessageSchema})
@require_permission("revenue_execution", "update")
...
@router.post("/playbooks/{playbook_id}/objections", response={201: dict, 400: MessageSchema, 404: MessageSchema})
@require_permission("revenue_execution", "create")
def create_sales_playbook_objection(request, playbook_id: int, payload: SalesPlaybookObjectionCreateSchema):
    try:
        playbook = get_object_or_404(_playbook_queryset(request), id=playbook_id)
...
@router.patch("/playbooks/objections/{objection_id}", response={200: dict, 400: MessageSchema, 404: MessageSchema})
@require_permission("revenue_execution", "update")
def update_sales_playbook_objection(request, objection_id: int, payload: SalesPlaybookObjectionUpdateSchema):
    try:
        objection = get_object_or_404(_playbook_objection_queryset(request), id=objection_id)
...
@router.delete("/playbooks/objections/{objection_id}", response={200: MessageSchema, 404: MessageSchema})
@require_permission("revenue_execution", "update")
...
@router.get("/okrs")
@require_permission("revenue_execution", "view")
def list_okrs(request, period_start: date = None, period_end: date = None, branch_id: int = None, status: str = None):
    start, end = _period_bounds(period_start, period_end)
    objectives = _objective_queryset(request).filter(period_start__lte=end, period_end__gte=start)
    if branch_id:
...
@router.post("/okrs", response={201: RevenueObjectiveOutSchema, 400: MessageSchema})
@require_permission("revenue_execution", "create")
...
@router.post("/okrs/{objective_id}/key-results", response={201: RevenueKeyResultOutSchema, 400: MessageSchema, 404: MessageSchema})
@require_permission("revenue_execution", "create")
def create_key_result(request, objective_id: int, payload: RevenueKeyResultCreateSchema):
    try:
...
@router.patch("/okrs/key-results/{key_result_id}", response={200: RevenueKeyResultOutSchema, 400: MessageSchema, 404: MessageSchema})
@require_permission("revenue_execution", "update")
def update_key_result(request, key_result_id: int, payload: RevenueKeyResultUpdateSchema):
    try:
        key_result = get_object_or_404(_key_result_queryset(request), id=key_result_id)
        return 200, _apply_key_result_payload(key_result, payload.dict(exclude_unset=True))
    except ValidationError as e:
        return 400, {"detail": _validation_detail(e)}
    except Exception as e:
        return 400, {"detail": str(e)}
...
@router.patch("/okrs/{objective_id}", response={200: RevenueObjectiveOutSchema, 400: MessageSchema, 404: MessageSchema})
@require_permission("revenue_execution", "update")
def update_okr(request, objective_id: int, payload: RevenueObjectiveUpdateSchema):
    try:
        objective = get_object_or_404(_objective_queryset(request), id=objective_id)
        return 200, _apply_objective_payload(objective, payload.dict(exclude_unset=True))
    except ValidationError as e:
        return 400, {"detail": _validation_detail(e)}
    except Exception as e:
        return 400, {"detail": str(e)}
...
def get_targets_summary(
    ...
    targets = with_target_progress(
        EmployeeTarget.objects.select_related("employee", "employee__user", "employee__branch", "role", "role_target_template")
    ...
    existing_template_ids = {row["template_id"] for row in target_rows if row["template_id"]}
    for template in templates.exclude(id__in=existing_template_ids).order_by("role__name", "sequence", "id"):
        target_rows.append({
            "id": None,
    ...
    return {
        ...
        "summary": {
            ...
            "target_progress_percentage": _decimal_pct(total_actual_value, total_target_value),
        },
    }


@router.get("/turnaround/plans", response=List[TurnaroundPlanOutSchema])
...
@router.post("/turnaround/plans", response={201: TurnaroundPlanOutSchema, 400: MessageSchema})
@require_permission("revenue_execution", "create")
...
@router.get("/turnaround/plans/active", response={200: TurnaroundPlanDetailSchema, 404: MessageSchema})
@require_permission("revenue_execution", "view")
...
@router.get("/turnaround/plans/{plan_id}", response={200: TurnaroundPlanDetailSchema, 404: MessageSchema})
@require_permission("revenue_execution", "view")
...
@router.patch("/turnaround/plans/{plan_id}", response={200: TurnaroundPlanOutSchema, 400: MessageSchema, 404: MessageSchema})
@require_permission("revenue_execution", "update")
def update_turnaround_plan(request, plan_id: int, payload: TurnaroundPlanUpdateSchema):
    try:
        plan = get_object_or_404(_turnaround_plan_queryset(request), id=plan_id)
        return 200, _apply_turnaround_plan_payload(plan, payload.dict(exclude_unset=True))
    except ValidationError as e:
        return 400, {"detail": _validation_detail(e)}
    except Exception as e:
        return 400, {"detail": str(e)}
...
@router.post("/turnaround/plans/{plan_id}/activate", response={200: TurnaroundPlanOutSchema, 400: MessageSchema, 404: MessageSchema})
@require_permission("revenue_execution", "update")
...
@router.post("/turnaround/plans/{plan_id}/close", response={200: TurnaroundPlanOutSchema, 400: MessageSchema, 404: MessageSchema})
@require_permission("revenue_execution", "update")
...
@router.patch("/turnaround/actions/{action_id}", response={200: TurnaroundActionOutSchema, 400: MessageSchema, 404: MessageSchema})
@require_permission("revenue_execution", "update")
def update_turnaround_action(request, action_id: int, payload: TurnaroundActionUpdateSchema):
    try:
        action = get_object_or_404(_turnaround_action_queryset(request), id=action_id)
        return 200, _apply_turnaround_action_payload(action, payload.dict(exclude_unset=True))
    except ValidationError as e:
        return 400, {"detail": _validation_detail(e)}
    except Exception as e:
        return 400, {"detail": str(e)}
...
@router.post("/turnaround/actions/{action_id}/complete", response={200: TurnaroundActionOutSchema, 400: MessageSchema, 404: MessageSchema})
@require_permission("revenue_execution", "complete")
def complete_turnaround_action(request, action_id: int, payload: TurnaroundActionCompleteSchema):
    try:
        action = get_object_or_404(_turnaround_action_queryset(request), id=action_id)
...
@router.post("/turnaround/actions/{action_id}/reopen", response={200: TurnaroundActionOutSchema, 400: MessageSchema, 404: MessageSchema})
@require_permission("revenue_execution", "complete")
...
@router.get("/turnaround/plans/{plan_id}/export")
@require_permission("revenue_execution", "view")
def export_turnaround_plan(request, plan_id: int):
    ...
    for action in plan.actions.order_by("sort_order", "week_start"):
        week = f"Week {action.week_start}" if action.week_start == action.week_end else f"Week {action.week_start}-{action.week_end}"
        rows.append([
    ...
    response = HttpResponse(csv_body, content_type="text/csv")
    response["Content-Disposition"] = 'attachment; filename="bomach-13-week-turnaround.csv"'
    return response


@router.get("/action-templates", response=List[DailyActionTemplateOutSchema])
@require_permission("revenue_execution", "list")
...
@router.post("/action-templates", response={201: DailyActionTemplateOutSchema, 400: MessageSchema})
@require_permission("revenue_execution", "create")
...
@router.patch("/action-templates/{template_id}", response={200: DailyActionTemplateOutSchema, 400: MessageSchema, 404: MessageSchema})
@require_permission("revenue_execution", "update")
def update_action_template(request, template_id: int, payload: DailyActionTemplateUpdateSchema):
    try:
...
@router.delete("/action-templates/{template_id}", response={200: MessageSchema, 404: MessageSchema})
...
@router.get("/days/today", response=DailyExecutionDayOutSchema)
@require_permission("revenue_execution", "view")
...
@router.post("/days/open", response={200: DailyExecutionDayOutSchema, 400: MessageSchema})
@require_permission("revenue_execution", "create")
...
@router.get("/days/{day_date}", response=DailyExecutionDayOutSchema)
@require_permission("revenue_execution", "view")
...
@router.patch("/actions/{action_id}", response={200: DailyActionInstanceOutSchema, 400: MessageSchema, 404: MessageSchema})
@require_permission("revenue_execution", "update")
...
@router.post("/actions/{action_id}/complete", response={200: DailyActionInstanceOutSchema, 400: MessageSchema, 404: MessageSchema})
@require_permission("revenue_execution", "complete")
...
@router.post("/actions/{action_id}/reopen", response={200: DailyActionInstanceOutSchema, 400: MessageSchema, 404: MessageSchema})
@require_permission("revenue_execution", "complete")
...
@router.get("/summary", response=DailyExecutionSummarySchema)
@require_permission("revenue_execution", "view")
def get_summary(request, date: date = None, branch_id: int = None):
    target_date = date or timezone.localdate()
    day = _day_queryset(request).filter(date=target_date, branch_id=branch_id).first()
    total, completed, open_count, completion_pct = _completion_counts(day) if day else (0, 0, 0, 0)
...
@router.get("/monthly-summary", response=MonthlyExecutionSummarySchema)
@require_permission("revenue_execution", "view")
def get_monthly_summary(request, month: str, branch_id: int = None):
    ...
    average_completion = round(sum(completion_values) / total_days, 2) if total_days else 0.0
...
@router.get("/speed-to-lead-queue", response=List[SpeedToLeadQueueItemSchema])
@require_permission("revenue_execution", "view")
def get_speed_to_lead_queue(request, branch_id: int = None, limit: int = 20):
    ...
    for lead in leads:
        ...
        should_include = (
            ...
            or (lead.status == "new" and not lead.first_response_at and not lead.first_contact_at)
            or lead.score >= 75
            or lead.is_stale
        )
...
@router.get("/activity-scorecard", response=List[ActivityScorecardRowSchema])
@require_permission("revenue_execution", "view")
def get_activity_scorecard(request, date: date = None, branch_id: int = None):
    target_date = date or timezone.localdate()
    start, end = _date_bounds(target_date)

    activities = _activity_queryset(request).filter(created_at__gte=start, created_at__lte=end)
    ...
    for activity in activities:
        ...
        employees.setdefault(label, {"activities": 0, "completed": 0, "assigned": 0, "sla_total": 0, "sla_done": 0})
    ...
    for label, metrics in sorted(employees.items()):
        action_score = round((metrics["completed"] / metrics["assigned"]) * 100) if metrics["assigned"] else 0
        activity_score = min(100, metrics["activities"] * 10)
        sla_score = round((metrics["sla_done"] / metrics["sla_total"]) * 100) if metrics["sla_total"] else 0
        score_parts = [value for value in [action_score, activity_score, sla_score] if value]
