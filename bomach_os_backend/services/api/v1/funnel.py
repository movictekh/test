...
from services.api.schema.crm_schemas import (
    FunnelSummarySchema, FunnelStageSummarySchema,
    ConversionBreakdownSchema, DropOffAlertSchema,
    FunnelLeadListSchema,
)
from services.models.crm import FunnelStage, FunnelLead, FunnelSnapshot
...
def get_funnel_summary(request):
    ...
    for stage in stages:
        ...
        if last_snapshot and last_snapshot.count > 0:
            change_pct = round(((current_count - last_snapshot.count) / last_snapshot.count) * 100, 2)
        
        result.append(FunnelStageSummarySchema(
            name=stage.name,
...
def get_lead_activity_log(
    ...
    leads = FunnelLead.objects.select_related('stage', 'assigned_role', 'assigned_role__user', 'branch')
