...
from services.api.schema.crm_schemas import (
    InquiryListSchema, InquirySummarySchema, FollowUpSchema,
    AssignAgentSchema, UpdateInquiryStatusSchema,
    CreateInquirySchema, UpdateInquirySchema,
    CreateFollowUpSchema, UpdateFollowUpSchema,
)
from services.models.crm import Inquiry, FollowUp
...
def get_inquiries(
    ...
    inquiries = Inquiry.objects.select_related('assigned_agent', 'assigned_agent__user', 'branch')
...
@csrc_router.patch("/csrc/inquiries/{inquiry_id}", response={200: InquiryListSchema, 400: dict, 404: dict})
...
def update_inquiry(request, inquiry_id: int, payload: UpdateInquirySchema):
    ...
    if 'assigned_agent_id' in update_data:
        if update_data['assigned_agent_id']:
            inquiry.assigned_agent = Employee.objects.get(id=update_data['assigned_agent_id'])
        else:
...
@csrc_router.post("/csrc/inquiries/{inquiry_id}/assign", response={200: InquiryListSchema, 400: dict, 404: dict})
@require_permission("leads", "update")
...
@csrc_router.patch("/csrc/inquiries/{inquiry_id}/status", response={200: InquiryListSchema, 400: dict, 404: dict})
@require_permission("leads", "update")
...
def get_followups(request, tab: str = 'today'):
    ...
    if tab == 'today':
        ...
        followups = FollowUp.objects.filter(
            ...
            scheduled_at__date=today + timedelta(days=1)
        )
    elif tab == 'overdue':
        followups = FollowUp.objects.filter(
            status='pending',
...
@csrc_router.patch("/csrc/followups/{followup_id}", response={200: FollowUpSchema, 400: dict, 404: dict})
@require_permission("leads", "update")
