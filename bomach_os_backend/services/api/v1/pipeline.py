...
from services.api.schema.crm_schemas import (
    DealListSchema, PipelineStageSchema, PipelineSummarySchema,
    PipelineReportSchema, CreateDealSchema, UpdateDealSchema, MoveDealStageSchema,
...
def get_deals(
    ...
    for pipeline_stage in PipelineStage.objects.all().order_by('order'):
        ...
        stages_data.append(PipelineStageSchema(
...
@pipeline_router.patch("/pipeline/deals/{deal_id}/stage", response={200: DealListSchema, 400: dict, 404: dict})
...
@pipeline_router.patch("/pipeline/deals/{deal_id}", response={200: DealListSchema, 400: dict, 404: dict})
@require_permission("leads", "update")
...
def get_pipeline_reports(request, period: int = 30):
    ...
    total_created = Deal.objects.filter(
