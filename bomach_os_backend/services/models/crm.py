...
class Lead(BaseModel):
    ...
    ACTIVE_STATUSES = [
        'new', 'contacted', 'qualified', 'proposal_sent', 'negotiation', 'dormant'
    ]
    SLA_STATUS_CHOICES = [
        ('safe', 'Safe'),
        ('due_now', 'Due Now'),
    ...
    sla_status = models.CharField(max_length=20, choices=SLA_STATUS_CHOICES, default='safe')
    ...
    def refresh_sla_status(self, now=None):
        ...
        elif self.first_response_due_at and now + timezone.timedelta(minutes=5) >= self.first_response_due_at:
    ...
    def refresh_score(self):
        ...
        value_bonus = min(10, int((self.estimated_value or Decimal('0')) / Decimal('3000000')))
...
class LeadActivity(BaseModel):
    ...
    next_follow_up_at = models.DateTimeField(null=True, blank=True)
    next_action = models.CharField(max_length=255, blank=True)
    from_status = models.CharField(max_length=30, choices=Lead.STATUS_CHOICES, blank=True)
    ...
    def create_for_lead(cls, lead_id, **kwargs):
        with transaction.atomic():
            ...
            sequence = (
                cls.objects.filter(lead=lead).aggregate(max_sequence=Max('sequence'))['max_sequence']
                or 0
            ) + 1
...
class LeadFunnelEvent(BaseModel):
    ...
    event_type = models.CharField(max_length=20, choices=EVENT_TYPE_CHOICES, default='transition')
...
class DailyActionInstance(BaseModel):
    ...
    severity = models.CharField(max_length=20, choices=DailyActionTemplate.SEVERITY_CHOICES, default='warning')
...
class RevenueObjective(BaseModel):
    ...
    def progress_percentage(self):
        ...
        if not key_results:
            return Decimal('0.00')
        if weighted_total > 0:
            progress = sum(kr.progress_percentage * kr.weight for kr in key_results) / weighted_total
        else:
            progress = sum(kr.progress_percentage for kr in key_results) / Decimal(len(key_results))
...
class RevenueKeyResult(BaseModel):
    ...
    progress_mode = models.CharField(max_length=30, choices=PROGRESS_MODE_CHOICES, default='manual')
    ...
    def clean(self):
        super().clean()
        if self.progress_mode == 'employee_target' and not self.linked_employee_target_id:
            raise ValidationError({'linked_employee_target': 'Linked employee target is required for employee target progress.'})
        if self.progress_mode == 'employee_kpi' and not self.linked_kpi_record_id:
            raise ValidationError({'linked_kpi_record': 'Linked KPI record is required for employee KPI progress.'})
    ...
    def effective_target_value(self):
        ...
        if self.progress_mode == 'employee_kpi' and self.linked_kpi_record and self.linked_kpi_record.target_value is not None:
...
class SalesPlaybook(BaseModel):
    ...
    def clean(self):
        ...
        if any(not isinstance(question, str) or not question.strip() for question in self.questions):
            raise ValidationError({'questions': 'Each question must be a non-empty string.'})
...
class FollowUp(BaseModel):
    ...
    schedule_type = models.CharField(max_length=20, choices=SCHEDULE_CHOICES, default='today')
