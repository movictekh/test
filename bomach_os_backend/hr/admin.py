...
from .models import (
    ...
    DailyWorkReport, Award, DisciplinaryCase, EmployeeEvaluation,
...
class JobPostingAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ('Basic Information', {
            'fields': ('job_title', 'department', 'branch', 'job_type', 'status', 'is_active')
        }),
        ('Job Details', {
            'fields': ('description', 'requirements', 'responsibilities', 'salary_min', 'salary_max', 'deadline')
        }),
        ('Timestamps', {
...
class PerformanceReviewAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ...
        ('Review Details', {
            'fields': ('review_date', 'review_period')
        }),
        ('Rating & Feedback', {
            'fields': ('overall_rating', 'rating_display', 'strengths', 'areas_for_improvement', 'feedback', 'employee_comment')
        }),
        ('Timestamps', {
            'fields': ('created_at', 'updated_at'),
            'classes': ('collapse',)
...
class EmployeeEvaluationAdmin(admin.ModelAdmin):
    ...
    list_filter = ['month', 'year', 'promotion_required', 'training_required', 'salary_increase']
    search_fields = []
    readonly_fields = ['created_at', 'updated_at']
    ...
    fieldsets = (
        ('Employee Information', {
            'fields': ('employee', 'evaluator')
        }),
        ('Period', {
            'fields': ('month', 'year', 'scorecard')
        }),
        ('Manager Input', {
...
class InterviewAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ('Interview Details', {
            'fields': ('applicant', 'interviewer', 'scheduled_at', 'meeting_link', 'status')
        }),
        ('Feedback', {
            'fields': ('feedback',)
        }),
...
class OfferLetterAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ('Offer Details', {
            'fields': ('applicant', 'template', 'annual_salary', 'start_date', 'status')
        }),
        ('Letter Content', {
