...
class EmployeeEvaluation(BaseModel):
    ...
    def clean(self):
        super().clean()
        if self.employee_id and self.evaluator_id and self.employee_id == self.evaluator_id:
            raise ValidationError("Evaluator cannot be the same as the employee being evaluated")
