...
class Applicant(BaseModel):
    ...
    job_posting = models.ForeignKey(
        JobPosting, on_delete=models.CASCADE, related_name="applicants"
    )
    ...
    status = models.CharField(
        max_length=50, choices=Status.choices, default=Status.APPLIED
    )

    highest_education = models.CharField(
        max_length=100, choices=Education.choices, blank=True, null=True
    )
    institution_name = models.CharField(max_length=255, blank=True, null=True)

    source = models.CharField(
        max_length=20, choices=Source.choices, blank=True, null=True
    )
