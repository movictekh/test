...
class DailyWorkReport(BaseModel):
    ...
    # Content
    operational_base = models.CharField(max_length=500, blank=True, null=True, help_text="Challenges faced today")
    work_activities = models.TextField(blank=True, null=True, help_text="Accomplishments today")
    task_details = models.TextField(blank=True, null=True, help_text="Details about tasks worked on")
    ...
    def clean(self):
        ...
        if self.status in ('approved', 'rejected'):
            if not self.reviewed_by_id or not self.reviewed_at:
                raise ValidationError("Decided work reports must include reviewer details.")
        elif self.reviewed_by_id or self.reviewed_at:
            raise ValidationError("Undecided work reports cannot include reviewer details.")

        if self.status == 'rejected' and not (self.feedback or '').strip():
            raise ValidationError({'feedback': "Feedback is required when rejecting a work report."})
