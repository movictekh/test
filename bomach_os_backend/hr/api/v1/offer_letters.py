...
@router.post(
    "/{applicant_id}/offer-letters/",
    response={201: OfferLetterResponseSchema, 400: MessageSchema},
)
...
def create_offer_letter(request, applicant_id: int, payload: OfferLetterCreateSchema):
    try:
        applicant = get_object_or_404(
            Applicant.objects.select_related("job_posting"), id=applicant_id
        )
...
@router.get(
    "/{applicant_id}/offer-letters/{offer_id}", response=OfferLetterResponseSchema
)
@require_permission("offer_letters", "view")
def get_offer_letter(request, applicant_id: int, offer_id: int):
    offer = get_object_or_404(OfferLetter, id=offer_id, applicant_id=applicant_id)
    return offer


@router.patch(
    "/{applicant_id}/offer-letters/{offer_id}",
    response={200: OfferLetterResponseSchema, 400: MessageSchema},
)
@require_permission("offer_letters", "update")
def update_offer_letter(
    request, applicant_id: int, offer_id: int, payload: OfferLetterUpdateSchema
):
    try:
        offer = get_object_or_404(OfferLetter, id=offer_id, applicant_id=applicant_id)
        ...
        if (
            any(k in update_data for k in ("template", "annual_salary", "start_date"))
            and "letter_content" not in update_data
        ):
            applicant = Applicant.objects.select_related("job_posting").get(
                id=applicant_id
            )
