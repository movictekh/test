...
def list_payroll(
    ...
    if filters.period_date_from:
        payroll_records = payroll_records.filter(period_date__gte=filters.period_date_from)
    if filters.period_date_to:
        payroll_records = payroll_records.filter(period_date__lte=filters.period_date_to)
    if filters.status:
    ...
    payroll_records = scope_queryset(request, payroll_records, owner_field="employee__user",
...
def process_payroll_batch(request, data: ProcessPayrollSchema):
    ...
    return 201, {"message": f"Successfully generated {processed_count} payroll records."}


@router.get("/make-payment/{year}/{month}", response={200: PayrollSummaryOut, 400: MessageSchema, 403: MessageSchema})
...
def make_payment(request, month:int, year:int):
    try:
        ...
        total_net = pending_payrolls.aggregate(Sum('net_salary'))['net_salary__sum'] or Decimal('0.00')
...
@router.post("/make-payment/{year}/{month}/authorize", response={200: dict, 400: MessageSchema, 403: MessageSchema})
...
def authorize_payroll(request, year:int, month:int):
    try:
        ...
        total_needed = payrolls.aggregate(Sum('net_salary'))['net_salary__sum'] or Decimal('0.00')

        if settings.COMPANY_WALLET_BALANCE < total_needed:
            return 400, {"detail": "Insufficient funds in company wallet to process payroll."}
