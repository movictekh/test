...
def _assign_advance_relations(request, advance, data):
    ...
    if "finance_account_id" in data:
        ...
        advance.finance_account = _get_scoped_cash_account(request, account_id) if account_id else None
    if "service_order_id" in data:
        order_id = data.pop("service_order_id")
        advance.service_order = _get_scoped_order(request, order_id) if order_id else None
...
def list_petty_cash_advances(
    ...
    if branch_id:
        advances = advances.filter(Q(branch_id=branch_id) | Q(finance_account__branch_id=branch_id) | Q(service_order__branch_id=branch_id))
    if service_order_id:
    ...
    if overdue is not None:
        ...
        advances = advances.filter(overdue_filter) if overdue else advances.exclude(overdue_filter)
    if search:
        q = search.strip()
...
@router.post("/petty-cash/advances", response={201: PettyCashAdvanceOut, 400: MessageSchema})
...
@router.get("/petty-cash/advances/{advance_id}", response={200: PettyCashAdvanceOut, 404: MessageSchema})
@require_permission("petty_cash", "view")
def get_petty_cash_advance(request, advance_id: int):
    advance = get_object_or_404(_apply_branch_scope(request, _advance_queryset()), id=advance_id)
    return 200, advance


@router.patch("/petty-cash/advances/{advance_id}", response={200: PettyCashAdvanceOut, 400: MessageSchema, 404: MessageSchema})
@require_permission("petty_cash", "update")
def update_petty_cash_advance(request, advance_id: int, payload: PettyCashAdvanceUpdate):
    try:
        advance = get_object_or_404(_apply_branch_scope(request, _advance_queryset()), id=advance_id)
        data = payload.dict(exclude_unset=True)
        if "status" in data:
            return 400, {"detail": "Use the petty cash workflow endpoints to change status."}
...
@router.post("/petty-cash/advances/{advance_id}/approve", response={200: PettyCashAdvanceOut, 400: MessageSchema, 404: MessageSchema})
...
@router.post("/petty-cash/advances/{advance_id}/reject", response={200: PettyCashAdvanceOut, 400: MessageSchema, 404: MessageSchema})
@require_permission("petty_cash", "reject")
def reject_petty_cash_advance_endpoint(request, advance_id: int, payload: PettyCashRejectIn):
    try:
        advance = get_object_or_404(_apply_branch_scope(request, _advance_queryset()), id=advance_id)
        rejected = reject_petty_cash_advance(advance, request.user, payload.rejection_reason)
        return 200, _advance_queryset().get(id=rejected.id)
    except Exception as exc:
        return 400, handle_payment_exception(exc)


@router.post("/petty-cash/advances/{advance_id}/issue", response={200: PettyCashAdvanceOut, 400: MessageSchema, 404: MessageSchema})
@require_permission("petty_cash", "issue")
def issue_petty_cash_advance_endpoint(request, advance_id: int, payload: PettyCashIssueIn):
    try:
        advance = get_object_or_404(_apply_branch_scope(request, _advance_queryset()), id=advance_id)
...
@router.post("/petty-cash/advances/{advance_id}/retire", response={200: PettyCashAdvanceOut, 400: MessageSchema, 404: MessageSchema})
@require_permission("petty_cash", "retire")
def retire_petty_cash_advance_endpoint(request, advance_id: int, payload: PettyCashRetireIn):
    try:
        advance = get_object_or_404(_apply_branch_scope(request, _advance_queryset()), id=advance_id)
...
@router.post("/petty-cash/advances/{advance_id}/cancel", response={200: PettyCashAdvanceOut, 400: MessageSchema, 404: MessageSchema})
...
@router.get("/petty-cash/advances/{advance_id}/retirement-lines", response=List[PettyCashRetirementLineOut])
@paginate(LimitOffsetPagination, page_size=10)
@require_permission("petty_cash", "view")
...
def petty_cash_summary(
    ...
    advances = _apply_branch_scope(request, _advance_queryset())
    accounts = FinanceAccount.objects.filter(account_type=FinanceAccount.ACCOUNT_TYPE.CASH, is_active=True)
    branch_ids = getattr(request, "_perm_branch_ids", [])
    if getattr(request, "_perm_scope", "branches") != "company" and branch_ids:
        accounts = accounts.filter(Q(branch_id__in=branch_ids) | Q(branch__isnull=True))
    if branch_id:
    ...
    open_statuses = [PettyCashAdvance.STATUS.ISSUED, PettyCashAdvance.STATUS.PARTIALLY_RETIRED]
    active_statuses = open_statuses + [PettyCashAdvance.STATUS.RETIRED]
    issued_total = _money(advances.filter(status__in=active_statuses).aggregate(total=Sum("amount_issued"))["total"])
    retired_total = _money(advances.filter(status__in=active_statuses).aggregate(total=Sum("amount_retired"))["total"])
    returned_total = _money(advances.filter(status__in=active_statuses).aggregate(total=Sum("amount_returned"))["total"])
    unretired_total = _money(sum(advance.unretired_amount for advance in advances.filter(status__in=open_statuses)))
    overdue_q = advances.filter(status__in=open_statuses, due_date__lt=timezone.localdate())
    ...
    for account in accounts.select_related("branch").order_by("display_name"):
        ...
        account_issued = _money(account_advances.filter(status__in=active_statuses).aggregate(total=Sum("amount_issued"))["total"])
        account_returned = _money(account_advances.filter(status__in=active_statuses).aggregate(total=Sum("amount_returned"))["total"])
        account_unretired = _money(sum(advance.unretired_amount for advance in account_advances.filter(status__in=open_statuses)))
        calculated_balance = _money(account.opening_balance - account_issued + account_returned)
        replenishment_needed = calculated_balance <= _money(account.opening_balance * Decimal("0.25"))
        ...
        account_rows.append(
            {
                ...
                "overdue_count": account_advances.filter(status__in=open_statuses, due_date__lt=timezone.localdate()).count(),
