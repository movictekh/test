...
def _journal_queryset():
    return JournalEntry.objects.select_related(
        "branch", "created_by", "posted_by", "reversal_of"
...
def _manual_branch(request, branch_id):
    ...
    if not is_company_scope and branch_ids and not branch_id:
        raise ValidationError(
            "Branch-scoped journal creation/update requires a branch_id."
        )
...
def list_ledger_accounts(
    ...
    if is_postable is not None:
        qs = qs.filter(is_postable=is_postable)
    if is_active is not None:
        qs = qs.filter(is_active=is_active)
    if system_role:
        qs = qs.filter(system_role=system_role)
    if search:
        q = search.strip()
...
def create_ledger_account(request, payload: LedgerAccountIn):
    try:
        ...
        account = LedgerAccount(**data, parent=parent, created_by=request.user)
        account.save()
...
@router.get(
    ...
    response={200: LedgerAccountOut, 404: MessageSchema},
)
...
@router.patch(
    "/ledger-accounts/{account_id}",
    response={200: LedgerAccountOut, 400: MessageSchema, 404: MessageSchema},
)
@require_permission("chart_of_accounts", "update")
def update_ledger_account(request, account_id: int, payload: LedgerAccountUpdate):
    try:
        account = get_object_or_404(_ledger_queryset(), id=account_id)
        data = payload.dict(exclude_unset=True)
        if "parent_id" in data:
            parent_id = data.pop("parent_id")
            account.parent = (
                get_object_or_404(LedgerAccount, id=parent_id) if parent_id else None
            )
...
@router.post(
    "/ledger-accounts/{account_id}/deactivate",
    response={200: LedgerAccountOut, 400: MessageSchema, 404: MessageSchema},
)
...
@router.post(
    "/accounts/{account_id}/ledger-account",
    response={200: FinanceAccountLedgerMapOut, 400: MessageSchema, 404: MessageSchema},
)
@require_permission("chart_of_accounts", "update")
def map_finance_account_endpoint(
    request, account_id: int, payload: FinanceAccountLedgerMapIn
):
    try:
        finance_account = get_object_or_404(
            scope_queryset(
                request,
                FinanceAccount.objects.select_related("branch", "ledger_account"),
        ...
        return 200, {
            "finance_account_id": mapped.id,
            "finance_account_name": mapped.display_name,
            "ledger_account_id": mapped.ledger_account_id,
            "ledger_account_code": mapped.ledger_account.code,
...
@router.get(
    "/journals/{journal_id}", response={200: JournalEntryDetailOut, 404: MessageSchema}
)
@require_permission("journals", "view")
def get_journal(request, journal_id: int):
    entry = get_object_or_404(
        _scope_journals(request, _journal_queryset()), id=journal_id
    )
...
@router.patch(
    "/journals/{journal_id}",
    response={200: JournalEntryDetailOut, 400: MessageSchema, 404: MessageSchema},
)
@require_permission("journals", "update")
def update_manual_journal_endpoint(
    request, journal_id: int, payload: ManualJournalUpdate
):
    try:
        entry = get_object_or_404(
            _scope_journals(request, _journal_queryset()), id=journal_id
        ...
        data = payload.dict(exclude_unset=True)
        branch_marker = "branch_id" in data
        branch = None
        if branch_marker:
            branch = _manual_branch(request, data.pop("branch_id"))
        lines = data.pop("lines", None)
        updated = update_manual_journal(
            entry,
            ...
            memo=data.get("memo"),
            lines=[line.dict() for line in lines] if lines is not None else None,
        )
...
@router.post(
    ...
    response={201: JournalEntryDetailOut, 400: MessageSchema, 404: MessageSchema},
)
...
def reverse_journal_endpoint(request, journal_id: int, payload: JournalReverseIn):
    try:
        ...
        reversal = reverse_journal_entry(
            entry, request.user, entry_date=payload.entry_date, memo=payload.memo
        )
...
def general_ledger(
    request,
    ledger_account_id: Optional[int] = Query(None),
    ...
    scoped = JournalLine.objects.select_related(
        "journal_entry", "journal_entry__branch", "ledger_account"
    ).filter(
    ...
    if date_from:
        ...
        for row in prior.values(
            "ledger_account_id", "ledger_account__normal_balance"
        ).annotate(total_debit=Sum("debit"), total_credit=Sum("credit")):
            debit, credit = _money(row["total_debit"]), _money(row["total_credit"])
            running[row["ledger_account_id"]] = (
                _money(debit - credit)
                if row["ledger_account__normal_balance"]
                == LedgerAccount.NORMAL_BALANCE.DEBIT
    ...
    rows = []
    for line in scoped.order_by(
        "journal_entry__entry_date", "journal_entry_id", "line_order"
    ):
        entry = line.journal_entry
        current = running.get(line.ledger_account_id, Decimal("0.00"))
        delta = (
            (line.debit - line.credit)
            if line.ledger_account.normal_balance == LedgerAccount.NORMAL_BALANCE.DEBIT
...
def trial_balance(
    ...
    lines = JournalLine.objects.filter(
        journal_entry__status=JournalEntry.STATUS.POSTED,
        journal_entry__entry_date__lte=balance_date,
        journal_entry__currency=currency,
    )
    branch_ids = getattr(request, "_perm_branch_ids", [])
    if getattr(request, "_perm_scope", "branches") != "company" and branch_ids:
