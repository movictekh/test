...
from finance.models import BankReconciliation,BankReconciliationMatch,BankStatementLine,FinanceAccount,JournalEntry,JournalLine
ZERO=Decimal('0.00'); CENT=Decimal('0.01')
def money(v): return (v or ZERO).quantize(CENT)
def _movement(line): return money(line.debit or line.credit)
def _norm(v): return re.sub(r'[^A-Z0-9]','',(v or '').upper())
def _match_q(rec):
    return Q(bank_reconciliation_id=rec.id)|Q(bank_reconciliation__finance_account_id=rec.finance_account_id,bank_reconciliation__status__in=[BankReconciliation.STATUS.RECONCILED,BankReconciliation.STATUS.CLOSED],bank_reconciliation__statement_end_date__lte=rec.statement_end_date)

def create_bank_reconciliation(*,finance_account,statement_start_date,statement_end_date,statement_opening_balance,statement_closing_balance,created_by,notes=''):
    with transaction.atomic():
        a=FinanceAccount.objects.select_for_update().select_related('ledger_account','branch').get(pk=finance_account.pk)
...
def add_bank_statement_lines(reconciliation,lines):
    with transaction.atomic():
        ...
        if r.status!=BankReconciliation.STATUS.DRAFT: raise ValidationError('Statement lines can only be added while reconciliation is draft.')
        out=[]
        for p in lines:
            x=BankStatementLine(bank_reconciliation=r,transaction_date=p['transaction_date'],value_date=p.get('value_date'),description=p.get('description','') or '',reference=p.get('reference','') or '',amount=money(p['amount']),direction=p['direction'],running_balance=p.get('running_balance'),external_transaction_id=p.get('external_transaction_id','') or '',sequence_number=p['sequence_number']); x.save(); out.append(x)
        return out

def statement_line_remaining(line): return money(line.amount-money(line.matches.aggregate(total=Sum('matched_amount'))['total']))
def journal_line_remaining(line): return money(_movement(line)-money(BankReconciliationMatch.objects.filter(journal_line_id=line.id).aggregate(total=Sum('matched_amount'))['total']))

def match_bank_statement_line(*,reconciliation,bank_statement_line,journal_line,matched_amount,matched_by,match_type=BankReconciliationMatch.MATCH_TYPE.MANUAL,notes=''):
    with transaction.atomic():
        r=BankReconciliation.objects.select_for_update().select_related('finance_account__ledger_account','finance_account__branch').get(pk=reconciliation.pk)
        if r.status!=BankReconciliation.STATUS.DRAFT: raise ValidationError('Matches can only change while reconciliation is draft.')
        s=BankStatementLine.objects.select_for_update().select_related('bank_reconciliation').get(pk=bank_statement_line.pk)
        jl=JournalLine.objects.select_for_update().select_related('journal_entry','ledger_account').get(pk=journal_line.pk)
        if s.bank_reconciliation_id!=r.id: raise ValidationError('Statement line does not belong to this reconciliation.')
        amount=money(matched_amount)
        if amount<=ZERO: raise ValidationError('Matched amount must be positive.')
        if amount>statement_line_remaining(s): raise ValidationError('Matched amount exceeds remaining statement amount.')
        if amount>journal_line_remaining(jl): raise ValidationError('Matched amount exceeds remaining journal-line movement.')
        m=BankReconciliationMatch(bank_reconciliation=r,bank_statement_line=s,journal_line=jl,matched_amount=amount,match_type=match_type,matched_by=matched_by,notes=notes or ''); m.save(); return m

def delete_bank_reconciliation_match(match):
    with transaction.atomic(): BankReconciliationMatch.objects.select_for_update().select_related('bank_reconciliation').get(pk=match.pk).delete()

def _book_balance(ledger_id,as_of):
    t=JournalLine.objects.filter(ledger_account_id=ledger_id,journal_entry__status=JournalEntry.STATUS.POSTED,journal_entry__entry_date__lte=as_of).aggregate(debit=Sum('debit'),credit=Sum('credit'))
...
def bank_gl_candidates(reconciliation,limit=250):
    ...
    if not r.finance_account.ledger_account_id: raise ValidationError('Reconciliation Finance account has no mapped ledger.')
    rows=[]
    qs=JournalLine.objects.filter(ledger_account_id=r.finance_account.ledger_account_id,journal_entry__status=JournalEntry.STATUS.POSTED,journal_entry__entry_date__lte=r.statement_end_date).exclude(journal_entry__source_type='finance_account',journal_entry__source_event='opening_balance').select_related('journal_entry').order_by('journal_entry__entry_date','journal_entry_id','line_order')
    for line in qs:
        rem=journal_line_remaining(line)
        if rem<=ZERO: continue
        rows.append({'journal_line_id':line.id,'journal_entry_id':line.journal_entry_id,'journal_number':line.journal_entry.journal_number,'entry_date':line.journal_entry.entry_date,'reference':line.journal_entry.reference,'description':line.description,'direction':BankStatementLine.DIRECTION.CREDIT if line.debit else BankStatementLine.DIRECTION.DEBIT,'movement_amount':_movement(line),'remaining_amount':rem})
...
def auto_match_bank_reconciliation(reconciliation,matched_by):
    ...
    if r.status!=BankReconciliation.STATUS.DRAFT: raise ValidationError('Auto-match is only available while reconciliation is draft.')
    candidates=bank_gl_candidates(r,2000); created=[]
    for s in r.statement_lines.order_by('sequence_number','id'):
        rem=statement_line_remaining(s)
        if rem<=ZERO: continue
        possible=[c for c in candidates if c['direction']==s.direction and money(c['remaining_amount'])==rem and abs((c['entry_date']-s.transaction_date).days)<=7]
        ref=_norm(s.reference)
        if ref: possible=[c for c in possible if _norm(c['reference'])==ref]
        else: possible=[c for c in possible if c['entry_date']==s.transaction_date]
        if len(possible)!=1: continue
        jl=JournalLine.objects.get(pk=possible[0]['journal_line_id']); created.append(match_bank_statement_line(reconciliation=r,bank_statement_line=s,journal_line=jl,matched_amount=rem,matched_by=matched_by,match_type=BankReconciliationMatch.MATCH_TYPE.AUTOMATIC)); candidates=[c for c in candidates if c['journal_line_id']!=jl.id]
    return created

def reconcile_bank_reconciliation(reconciliation,reconciled_by):
    with transaction.atomic():
        r=BankReconciliation.objects.select_for_update().get(pk=reconciliation.pk)
        if r.status!=BankReconciliation.STATUS.DRAFT: raise ValidationError('Only draft reconciliations can be reconciled.')
        s=reconciliation_summary(r)
        if s['statement_internal_difference']!=ZERO: raise ValidationError('Statement opening balance plus statement movements does not equal statement closing balance.')
...
def close_bank_reconciliation(reconciliation,closed_by):
    with transaction.atomic():
        r=BankReconciliation.objects.select_for_update().get(pk=reconciliation.pk)
        if r.status!=BankReconciliation.STATUS.RECONCILED: raise ValidationError('Only reconciled bank reconciliations can be closed.')
        r.status=BankReconciliation.STATUS.CLOSED; r.closed_by=closed_by; r.closed_at=timezone.now(); r._workflow_via_service=True; r.save(update_fields=['status','closed_by','closed_at','updated_at']); return r
