...
class LedgerAccount(models.Model):
    ...
    system_role = models.CharField(
        max_length=50, choices=SYSTEM_ROLE.choices, null=True, blank=True, unique=True
    )
    ...
    def clean(self):
        ...
        if self.parent_id:
            if self.pk and self.parent_id == self.pk:
                ...
                errors["parent"] = (
                    "Parent and child ledger accounts must have the same account type."
                )
                ...
                while cursor:
                    if cursor.pk in seen:
                        errors["parent"] = (
                            "Ledger account hierarchy cannot contain a cycle."
                        )
                        break
                    seen.add(cursor.pk)
                    cursor = cursor.parent
        if self.pk and self.is_postable and self.children.exists():
            errors["is_postable"] = (
                "Ledger accounts with child accounts must remain non-postable."
            )
        if self.system_role and not self.is_postable:
            errors["system_role"] = "System-role ledger accounts must be postable."
        if self.system_role:
            expected_role_shape = {
                self.SYSTEM_ROLE.ACCOUNTS_RECEIVABLE: (
                    self.ACCOUNT_TYPE.ASSET,
                    self.NORMAL_BALANCE.DEBIT,
                ),
                self.SYSTEM_ROLE.ACCOUNTS_PAYABLE: (
                    self.ACCOUNT_TYPE.LIABILITY,
                    self.NORMAL_BALANCE.CREDIT,
                ),
                self.SYSTEM_ROLE.EMPLOYEE_RECEIVABLES: (
                    self.ACCOUNT_TYPE.ASSET,
                    self.NORMAL_BALANCE.DEBIT,
                ),
                self.SYSTEM_ROLE.SERVICE_REVENUE: (
                    self.ACCOUNT_TYPE.REVENUE,
                    self.NORMAL_BALANCE.CREDIT,
                ),
                self.SYSTEM_ROLE.SERVICE_COST_EXPENSE: (
                    self.ACCOUNT_TYPE.EXPENSE,
                    self.NORMAL_BALANCE.DEBIT,
                ),
                self.SYSTEM_ROLE.OPERATING_EXPENSE: (
                    self.ACCOUNT_TYPE.EXPENSE,
                    self.NORMAL_BALANCE.DEBIT,
                ),
                self.SYSTEM_ROLE.CAPITAL_EXPENDITURE_CLEARING: (
                    self.ACCOUNT_TYPE.ASSET,
                    self.NORMAL_BALANCE.DEBIT,
                ),
                self.SYSTEM_ROLE.PAYROLL_EXPENSE: (
                    self.ACCOUNT_TYPE.EXPENSE,
                    self.NORMAL_BALANCE.DEBIT,
                ),
                self.SYSTEM_ROLE.PAYROLL_DEDUCTIONS_PAYABLE: (
                    self.ACCOUNT_TYPE.LIABILITY,
                    self.NORMAL_BALANCE.CREDIT,
                ),
                self.SYSTEM_ROLE.STATUTORY_PAYABLE: (
                    self.ACCOUNT_TYPE.LIABILITY,
                    self.NORMAL_BALANCE.CREDIT,
                ),
                self.SYSTEM_ROLE.PETTY_CASH_ADVANCE: (
                    self.ACCOUNT_TYPE.ASSET,
                    self.NORMAL_BALANCE.DEBIT,
                ),
                self.SYSTEM_ROLE.OPENING_BALANCE_EQUITY: (
                    self.ACCOUNT_TYPE.EQUITY,
                    self.NORMAL_BALANCE.CREDIT,
                ),
                self.SYSTEM_ROLE.ASSET_DISPOSAL_GAIN: (
                    self.ACCOUNT_TYPE.REVENUE,
                    self.NORMAL_BALANCE.CREDIT,
                ),
                self.SYSTEM_ROLE.ASSET_DISPOSAL_LOSS: (
                    self.ACCOUNT_TYPE.EXPENSE,
                    self.NORMAL_BALANCE.DEBIT,
                ),
            ...
            if (
                expected_role_shape
                and (self.account_type, self.normal_balance) != expected_role_shape
            ):
                errors["system_role"] = (
                    "This system role is incompatible with the ledger account type/normal balance."
                )
        if self.system_role and not self.is_active:
            errors["is_active"] = (
                "Move the system role before deactivating this ledger account."
            )
        if (
            self.pk
            and not self.is_active
            and self.children.filter(is_active=True).exists()
        ):
        ...
        if self.pk and not self.is_active:
            ...
            if mapped_finance_account and mapped_finance_account.is_active:
                errors["is_active"] = (
                    "Deactivate or remap the active Finance account before deactivating its ledger account."
                )
        if self.pk:
            ...
            if original and original.has_finance_account_mapping():
                for field, label in structural_fields.items():
                    if getattr(self, field) != getattr(original, field):
                        errors[field] = (
                            f"{label} cannot change while this account tree contains a mapped Finance account."
                        )
            if original and original.has_posted_activity():
                for field, label in structural_fields.items():
                    if getattr(self, field) != getattr(original, field):
                        errors[field] = (
                            f"{label} cannot change after posted activity exists in this account or its descendants."
                        )
        if errors:
...
class JournalEntry(models.Model):
    ...
    def total_debit(self):
        return sum((line.debit for line in self.lines.all()), Decimal("0.00")).quantize(
            Decimal("0.01")
        )
    ...
    def total_credit(self):
        return sum(
            (line.credit for line in self.lines.all()), Decimal("0.00")
    ...
    def clean(self):
        ...
        source_parts = [
            ...
            bool(self.source_id),
            bool(self.source_event),
        ]
        if any(source_parts) and not all(source_parts):
            errors["source_type"] = (
                "Source journals require source_type, source_id and source_event together."
            )
        if self.entry_type in {
            self.ENTRY_TYPE.AUTOMATIC,
            self.ENTRY_TYPE.OPENING,
            self.ENTRY_TYPE.REVERSAL,
        } and not all(source_parts):
            errors["source_type"] = (
                "Automatic, opening and reversal journals require a complete source identity."
            )
        if self.entry_type == self.ENTRY_TYPE.MANUAL and any(source_parts):
            errors["source_type"] = (
                "Manual journals must not impersonate an automatic source event."
            )
        if self.entry_type == self.ENTRY_TYPE.REVERSAL and not self.reversal_of_id:
            errors["reversal_of"] = (
                "Reversal journals must identify the original journal."
            )
        ...
        if not self.pk and self.status == self.STATUS.POSTED:
            errors["status"] = (
                "Create journals as draft and post them through the accounting posting service."
            )
        if self.pk:
            original = type(self).objects.filter(pk=self.pk).first()
            if original and original.status == self.STATUS.POSTED:
                for field in [
                    ...
                    if getattr(self, field) != getattr(original, field):
                        errors[field] = (
                            "Posted journals are immutable. Use a reversal instead."
                        )
            if (
                original
                and original.status == self.STATUS.DRAFT
                and self.status == self.STATUS.POSTED
                and not getattr(self, "_posting_via_service", False)
            ):
                errors["status"] = (
                    "Journals can only be posted through the accounting posting service."
                )
        if errors:
            raise ValidationError(errors)
...
class JournalLine(models.Model):
    ...
    def clean(self):
        ...
        if bool(self.debit) == bool(self.credit):
            errors["debit"] = (
                "Each journal line must contain exactly one positive debit or one positive credit."
            )
        if self.ledger_account_id:
            if not self.ledger_account.is_active:
                errors["ledger_account"] = (
                    "Journal lines require an active ledger account."
                )
            elif not self.ledger_account.is_postable:
                errors["ledger_account"] = (
                    "Journal lines cannot post to a parent/non-postable account."
                )
            try:
            ...
            if finance_account and self.journal_entry_id:
                if (
                    finance_account.currency.upper()
                    != self.journal_entry.currency.upper()
                ):
                    errors["ledger_account"] = (
                        "Cash/bank ledger currency must match the journal currency."
                    )
                if (
                    finance_account.branch_id
                    and self.journal_entry.branch_id != finance_account.branch_id
                ):
                    errors["ledger_account"] = (
                        "Cash/bank ledger branch must exactly match the journal branch."
                    )
        if errors:
            raise ValidationError(errors)
    ...
    def save(self, *args, **kwargs):
        if (
            ...
            and self.journal_entry.status == JournalEntry.STATUS.POSTED
        ):
