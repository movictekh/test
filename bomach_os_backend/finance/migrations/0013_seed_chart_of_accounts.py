...
def seed_chart_and_map_finance_accounts(apps, schema_editor):
    ...
    for finance_account in FinanceAccount.objects.filter(
        ledger_account__isnull=True
    ).order_by("id"):
        ...
        FinanceAccount.objects.filter(pk=finance_account.pk).update(
            ledger_account_id=ledger.pk
        )
...
class Migration(migrations.Migration):
    ...
    operations = [
        migrations.RunPython(
            seed_chart_and_map_finance_accounts, migrations.RunPython.noop
        )
    ]
