...
class AccountingGLPassTests(RoleAPITestMixin, TestCase):
    def setUp(self):
        ...
        self.employee = self.create_user_with_employee(
            "fin.at23@test.com", "finat23", "EMP-FIN-AT23", role=self.role
        )
        self.headers = self.auth_headers(self.employee)
        self.branch = Branch.objects.create(
            branch_name="FIN AT23 Enugu",
            branch_id="BR-FIN-AT23",
            country="Nigeria",
            state="Enugu",
            office_address="Accounting test office",
            contact_email="fin-at23@test.com",
            contact_phone="+2348011111188",
        )
        self.finance_account = FinanceAccount.objects.create(
            account_type=FinanceAccount.ACCOUNT_TYPE.BANK,
            display_name="FIN AT23 Test Bank",
            currency="NGN",
            branch=self.branch,
            bank_name="GTBank",
    ...
    def _bank(self):
        return ensure_finance_account_ledger_account(
            self.finance_account, self.employee.user
        )

    def test_canonical_chart_is_seeded(self):
        roles = set(
            LedgerAccount.objects.exclude(system_role__isnull=True).values_list(
                "system_role", flat=True
            )
        )
        for role in [
            "service_revenue",
        ...
        self.assertTrue(
            LedgerAccount.objects.filter(code="1100", is_postable=False).exists()
        )
    ...
    def test_opening_balance_posts_once_and_locks_historical_identity(self):
        first, created1 = post_opening_balance_journal(
            self.finance_account, self.employee.user
        )
        second, created2 = post_opening_balance_journal(
            self.finance_account, self.employee.user
        )
        self.assertTrue(created1)
        self.assertFalse(created2)
        self.assertEqual(first.id, second.id)
        self.assertEqual(first.total_debit, Decimal("1000.00"))
        self.assertEqual(first.total_credit, Decimal("1000.00"))
        self.finance_account.opening_balance = Decimal("999.00")
        with self.assertRaises(ValidationError):
            self.finance_account.save()

    def test_parent_must_be_non_postable_and_hierarchy_cannot_cycle(self):
        parent = LedgerAccount.objects.create(
            code="6990",
        ...
        child = LedgerAccount.objects.create(
            code="6991",
    ...
    def test_unbalanced_manual_journal_cannot_post(self):
        revenue = LedgerAccount.objects.get(system_role="service_revenue")
        bank = self._bank()
        ...
        with self.assertRaises(ValidationError):
            post_journal_entry(entry, self.employee.user)
    ...
    def test_reversal_swaps_debits_and_credits(self):
        ...
        posted = post_journal_entry(entry, self.employee.user)
        reversal = reverse_journal_entry(posted, self.employee.user)
        self.assertEqual(reversal.entry_type, JournalEntry.ENTRY_TYPE.REVERSAL)
        self.assertEqual(reversal.total_debit, Decimal("250.00"))
        self.assertEqual(reversal.total_credit, Decimal("250.00"))
    ...
    def test_vendor_bill_posting_is_idempotent_and_splits_wht(self):
        vendor = FinanceVendor.objects.create(
            name="FIN AT23 Vendor", created_by=self.employee.user
        )
        bill = VendorBill.objects.create(
            ...
            category="Testing",
            description="GL vendor bill",
            gross_amount=Decimal("1000.00"),
            withholding_tax=Decimal("50.00"),
            bill_date=timezone.localdate(),
            due_date=timezone.localdate(),
            status=VendorBill.STATUS.PAID,
            paid_at=timezone.now(),
            payment_reference="AT23-VENDOR-001",
            paid_by=self.employee.user,
            created_by=self.employee.user,
        )
        first, c1 = post_vendor_bill_payment_journal(bill, self.employee.user)
        second, c2 = post_vendor_bill_payment_journal(bill, self.employee.user)
        self.assertTrue(c1)
        self.assertFalse(c2)
        self.assertEqual(first.id, second.id)
        self.assertEqual(first.total_debit, Decimal("1000.00"))
        self.assertEqual(first.total_credit, Decimal("1000.00"))
        self.assertEqual(
            first.lines.get(ledger_account__system_role="statutory_payable").credit,
            Decimal("50.00"),
        )
    ...
    def test_manual_journal_api_posts_and_trial_balance_balances(self):
        ...
        posted = self.client.post(
            f"/api/v1/finance/journals/{journal_id}/post", **self.headers
        )
        self.assertEqual(posted.status_code, 200)
        self.assertEqual(posted.json()["status"], "posted")
        trial = self.client.get(
            "/api/v1/finance/trial-balance?currency=NGN", **self.headers
        )
        self.assertEqual(trial.status_code, 200)
        self.assertTrue(trial.json()["balanced"])
        self.assertEqual(trial.json()["total_debit"], "75.00")
        self.assertEqual(trial.json()["total_credit"], "75.00")
    ...
    def test_finance_account_mapping_must_stay_inside_cash_bank_tree(self):
        ...
        with self.assertRaises(ValidationError):
            map_finance_account_ledger(
                self.finance_account, unrelated_asset, self.employee.user
            )
    ...
    def test_reversal_cannot_predate_original(self):
        ...
        with self.assertRaises(ValidationError):
            reverse_journal_entry(
                posted, self.employee.user, entry_date=original_date - timedelta(days=1)
            )

    def test_partial_payment_tax_rounding_reconciles_to_invoice_tax(self):
        customer_user = User.objects.create_user(
            email="tax.rounding@test.com",
            username="taxrounding",
