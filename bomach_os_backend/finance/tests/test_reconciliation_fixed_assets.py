...
class ReconciliationFixedAssetsPassTests(RoleAPITestMixin, TestCase):
    ...
    def _capex(self, amount=Decimal("100.00"), paid_date=date(2026, 4, 1)):
        e = Expense.objects.create(
            ...
            payment_reference=f"CAPEX-{amount}",
        )
        post_expense_payment_journal(e, self.employee.user)
        return e

    def test_reconciliation_requires_bank_account(self):
        cash = FinanceAccount.objects.create(
            account_type=FinanceAccount.ACCOUNT_TYPE.CASH,
            display_name="AT45 Cash",
            currency="NGN",
            opening_balance=Decimal("0.00"),
            created_by=self.employee.user,
        )
        ensure_finance_account_ledger_account(cash, self.employee.user)
        with self.assertRaises(ValidationError):
            create_bank_reconciliation(
                finance_account=cash,
                statement_start_date=timezone.localdate(),
                statement_end_date=timezone.localdate(),
    ...
    def test_reconciliation_supports_outstanding_gl_item(self):
        ...
        dep = self._bank_move(Decimal("100.00"), True, day, "DEP-100")
        self._bank_move(Decimal("20.00"), False, day, "PAY-20")
        r = create_bank_reconciliation(
            finance_account=self.finance_account,
            statement_start_date=day,
            statement_end_date=day,
            statement_opening_balance=Decimal("0.00"),
            statement_closing_balance=Decimal("100.00"),
            created_by=self.employee.user,
        )
        s = add_bank_statement_lines(
            r,
            [
                {
        ...
        match_bank_statement_line(
            ...
            journal_line=dep.lines.get(ledger_account=self.bank_ledger),
            matched_amount=Decimal("100.00"),
            matched_by=self.employee.user,
        )
        summary = reconciliation_summary(r)
        self.assertEqual(summary["outstanding_gl_net"], Decimal("-20.00"))
        self.assertEqual(summary["adjusted_statement_balance"], Decimal("80.00"))
        self.assertEqual(summary["book_closing_balance"], Decimal("80.00"))
        self.assertEqual(summary["unexplained_difference"], Decimal("0.00"))
        self.assertEqual(
            reconcile_bank_reconciliation(r, self.employee.user).status,
    ...
    def test_unmatched_statement_blocks_reconcile(self):
        ...
        with self.assertRaises(ValidationError):
            reconcile_bank_reconciliation(r, self.employee.user)

    def test_closed_reconciliation_is_immutable(self):
        day = timezone.localdate()
        dep = self._bank_move(Decimal("50.00"), True, day)
        r = create_bank_reconciliation(
            finance_account=self.finance_account,
            statement_start_date=day,
            statement_end_date=day,
            statement_opening_balance=Decimal("0.00"),
            statement_closing_balance=Decimal("50.00"),
            created_by=self.employee.user,
        )
        s = add_bank_statement_lines(
            r,
        ...
        r = close_bank_reconciliation(
            reconcile_bank_reconciliation(r, self.employee.user), self.employee.user
        )
        r.notes = "changed"
        with self.assertRaises(ValidationError):
            r.save()
    ...
    def test_category_enforces_accumulated_depreciation_shape(self):
        with self.assertRaises(ValidationError):
            FixedAssetCategory.objects.create(
                code="BAD",
                name="Bad",
                asset_ledger_account=LedgerAccount.objects.get(code="1610"),
                accumulated_depreciation_ledger_account=LedgerAccount.objects.get(
                ...
                depreciation_expense_ledger_account=LedgerAccount.objects.get(
                    code="6300"
                ),
    ...
    def test_capitalization_clears_capex(self):
        e = self._capex(Decimal("120.00"))
        c = self._category(12)
        a = create_fixed_asset(
            category=c,
            source_expense=e,
            name="Laptop",
            acquisition_date=e.date,
            acquisition_cost=Decimal("120.00"),
            created_by=self.employee.user,
        )
        a, j, created = capitalize_fixed_asset(a, self.employee.user)
        self.assertTrue(created)
        self.assertEqual(a.status, FixedAsset.STATUS.ACTIVE)
        self.assertEqual(
            j.lines.get(ledger_account__code="1610").debit, Decimal("120.00")
        )
        self.assertEqual(
            j.lines.get(
                ledger_account__system_role=LedgerAccount.SYSTEM_ROLE.CAPITAL_EXPENDITURE_CLEARING
            ).credit,
            Decimal("120.00"),
        )

    def test_source_expense_cannot_be_overcapitalized(self):
        e = self._capex(Decimal("100.00"))
        c = self._category(12)
        a = create_fixed_asset(
            category=c,
            source_expense=e,
            name="A",
            acquisition_date=e.date,
            acquisition_cost=Decimal("70.00"),
            created_by=self.employee.user,
        )
        capitalize_fixed_asset(a, self.employee.user)
        b = create_fixed_asset(
            category=c,
            source_expense=e,
            name="B",
            acquisition_date=e.date,
            acquisition_cost=Decimal("40.00"),
            created_by=self.employee.user,
        )
        with self.assertRaises(ValidationError):
            capitalize_fixed_asset(b, self.employee.user)
    ...
    def test_depreciation_first_full_month_and_cumulative_rounding(self):
        ...
        a, _, _ = capitalize_fixed_asset(
            a, self.employee.user, capitalization_date=date(2026, 4, 1)
        )
        sch = depreciation_schedule(a)
        self.assertEqual(sch[0]["period_end"], date(2026, 5, 31))
        self.assertEqual(
            [x["depreciation_amount"] for x in sch],
            [Decimal("33.33"), Decimal("33.34"), Decimal("33.33")],
        )
        posted = []
        for d in [date(2026, 5, 31), date(2026, 6, 30), date(2026, 7, 31)]:
            a, j, created = post_fixed_asset_depreciation(a, d, self.employee.user)
            self.assertTrue(created)
            posted.append(
                j.lines.get(
                    ledger_account=a.accumulated_depreciation_ledger_account
                ).credit
            )
            a.refresh_from_db()
        self.assertEqual(sum(posted, Decimal("0.00")), Decimal("100.00"))
        self.assertEqual(a.status, FixedAsset.STATUS.FULLY_DEPRECIATED)
    ...
    def test_disposal_gain_and_generic_reversal_block(self):
        ...
        a, cap, _ = capitalize_fixed_asset(a, self.employee.user)
        with self.assertRaises(ValidationError):
            reverse_journal_entry(cap, self.employee.user)
        a, j, created = dispose_fixed_asset(
            a,
        ...
        self.assertTrue(created)
        self.assertEqual(j.total_debit, Decimal("120.00"))
        self.assertEqual(j.total_credit, Decimal("120.00"))
        self.assertEqual(
            j.lines.get(
                ledger_account__system_role=LedgerAccount.SYSTEM_ROLE.ASSET_DISPOSAL_GAIN
            ).credit,
            Decimal("20.00"),
        )
        self.assertEqual(a.status, FixedAsset.STATUS.DISPOSED)

    def test_bank_reconciliation_api_is_deferred_while_fixed_assets_remain_mounted(
        self,
    ):
        ...
        self.assertEqual(
            self.client.get(
                "/api/v1/finance/bank-reconciliations", **self.headers
            ).status_code,
            404,
        )
        self.assertEqual(
            self.client.get(
                "/api/v1/finance/fixed-asset-categories", **self.headers
            ).status_code,
            200,
        )
        self.assertEqual(
