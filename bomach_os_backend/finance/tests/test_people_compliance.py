...
class FinancePayrollAPITests(RoleAPITestMixin, TestCase):
    ...
    def test_calculate_creates_company_run_lines_and_structured_items(self):
        ...
        response = self._post(f"/api/v1/finance/payroll/{run_id}/calculate")
        ...
        employee_items = [
            item for item in ada_line["items"] if item["source_type"] == "employee"
        ]
    ...
    def test_manual_items_update_totals_and_survive_recalculation(self):
        ...
        calculated = self._post(f"/api/v1/finance/payroll/{run_id}/calculate").json()
        ...
        recalculated = self._post(f"/api/v1/finance/payroll/{run_id}/calculate")
        ...
        manual_items = [
            item for item in ada_line["items"] if item["source_type"] == "manual"
        ]
    ...
    def test_submit_approve_pay_posts_single_payroll_cashbook_outflow(self):
        ...
        submitted = self._post(f"/api/v1/finance/payroll/{run_id}/submit")
        self.assertEqual(submitted.status_code, 200)
        self.assertEqual(submitted.json()["status"], "awaiting_approval")

        approved = self._post(f"/api/v1/finance/payroll/{run_id}/approve")
    ...
    def test_branch_run_calculates_only_branch_employees_and_scope_is_enforced(self):
        ...
        calculated = self._post(f"/api/v1/finance/payroll/{run_id}/calculate")
...
class FinanceCommissionsAPITests(RoleAPITestMixin, TestCase):
    ...
    def test_approved_commission_and_bonus_are_picked_up_by_payroll(self):
        ...
        incentive_items = [
            item for item in sales_line["items"] if item["source_type"] == "commission"
        ]
...
class FinanceStatutoryAPITests(RoleAPITestMixin, TestCase):
    def setUp(self):
        ...
        self.employee = self.create_user_with_employee(
            "stat.manager@test.com", "statmanager", "EMP-STAT-MGR", role=self.role
        )
        self.headers = self.auth_headers(self.employee)
        self.branch = Branch.objects.create(
            branch_name="Enugu",
            branch_id="BR-STAT-ENU",
            country="Nigeria",
            state="Enugu",
            office_address="Enugu office",
            contact_email="stat-enugu@test.com",
            contact_phone="+2348010000001",
        )
        ...
        self.account = FinanceAccount.objects.create(
            account_type="bank",
            display_name="Statutory Account",
            bank_name="GTBank",
    ...
    def post(self, path, payload=None):
        return self.client.post(
            path, data=payload or {}, content_type="application/json", **self.headers
        )
    ...
    def test_manual_vat_drives_summary(self):
        r = self.post(
            "/api/v1/finance/statutory/obligations",
            {
                ...
                "basis": "Reviewed VAT return",
                "basis_amount": "12000000.00",
                "amount": "918750.00",
                "due_date": "2026-08-21",
        ...
        s = self.client.get("/api/v1/finance/statutory/summary", **self.headers)
        self.assertEqual(s.status_code, 200)
        self.assertEqual(s.json()["vat_payable"], "918750.00")
    ...
    def test_wht_generation_and_cashbook_split(self):
        bill = VendorBill.objects.create(
            ...
            withholding_tax=Decimal("50000.00"),
            bill_date=date(2026, 8, 1),
            due_date=date(2026, 8, 10),
            status="paid",
            paid_by=self.employee.user,
            paid_at="2026-08-10T10:00:00+01:00",
            payment_reference="VEN-NET-001",
            created_by=self.employee.user,
        )
        g = self.post(
        ...
        self.assertEqual(g.json()["amount"], "50000.00")
        self.assertEqual(
            self.post(
                "/api/v1/finance/statutory/generate/wht",
                {
        ...
        oid = g.json()["id"]
        self.post(f"/api/v1/finance/statutory/obligations/{oid}/submit")
        self.post(f"/api/v1/finance/statutory/obligations/{oid}/approve")
        self.assertEqual(
            self.post(
                f"/api/v1/finance/statutory/obligations/{oid}/pay",
                {
                    "finance_account_id": self.account.id,
                    "paid_at": "2026-09-21T10:00:00+01:00",
                    "payment_reference": "WHT-REM-001",
                },
            ).status_code,
            200,
        )
        ...
        self.assertEqual(v.json()["items"][0]["money_out"], "950000.00")
        st = self.client.get(
            "/api/v1/finance/cashbook",
            {"date_from": "2026-09-21", "date_to": "2026-09-21", "source": "statutory"},
            **self.headers,
        )
        self.assertEqual(st.json()["items"][0]["money_out"], "50000.00")
    ...
    def test_payroll_paye_uses_explicit_deduction(self):
        ...
        line = c.json()["lines"][0]
        a = self.client.put(
            f"/api/v1/finance/payroll/{rid}/lines/{line['id']}/manual-items",
            data={
                "items": [
                    {
                        "item_type": "deduction",
        ...
        self.post(f"/api/v1/finance/payroll/{rid}/submit")
        self.post(f"/api/v1/finance/payroll/{rid}/approve")
        g = self.post(
            "/api/v1/finance/statutory/generate/payroll",
            {"payroll_run_id": rid, "category": "paye", "due_date": "2026-09-10"},
        )
        self.assertEqual(g.status_code, 201)
        self.assertEqual(g.json()["amount"], "25000.00")
        self.assertEqual(
            self.post(
                "/api/v1/finance/statutory/generate/payroll",
                {"payroll_run_id": rid, "category": "paye", "due_date": "2026-09-10"},
    ...
    def test_permission_is_separate(self):
        role = Role.objects.create(
            name="No Stat", permissions={"finance_payroll": ["list"]}
        )
        emp = self.create_user_with_employee(
            "nostat@test.com", "nostat", "EMP-NOSTAT", role=role
        )
        self.assertEqual(
            self.client.get(
                "/api/v1/finance/statutory/obligations", **self.auth_headers(emp)
            ).status_code,
            403,
        )
    ...
    def test_paid_cannot_be_voided(self):
        o = StatutoryObligation.objects.create(
            ...
            basis="Filing",
            basis_amount=Decimal("100000.00"),
        ...
        self.assertEqual(
            self.post(
                f"/api/v1/finance/statutory/obligations/{o.id}/pay",
                {
                    "finance_account_id": self.account.id,
                    "payment_reference": "STAT-001",
                },
            ).status_code,
            200,
        )
        self.assertEqual(
            self.post(f"/api/v1/finance/statutory/obligations/{o.id}/void").status_code,
            400,
        )
