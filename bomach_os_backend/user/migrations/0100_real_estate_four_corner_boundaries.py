...
def _to_boundary(value):
    ...
    if isinstance(value, dict):
        unknown = sorted(set(value) - set(CORNERS))
        if unknown:
            raise RuntimeError(
                "Existing real-estate boundary contains unsupported named keys: "
                + ", ".join(unknown)
            )
        return {
            key: point
    ...
    if isinstance(value, list):
        if len(value) > 4:
            raise RuntimeError(
                "Existing real-estate boundary has more than four points; "
                "refusing to truncate geographic data."
            )
        return {
            corner: point
    ...
    raise RuntimeError(
        f"Unsupported existing real-estate boundary value type: {type(value).__name__}"
    )
    ...
    operations = [
        migrations.AddField(
            ...
            field=models.JSONField(
                ...
                default=dict,
                help_text=(
                    "Optional named corners: nw, ne, se, sw. "
                    "Any subset from zero to four corners is valid."
                ),
        ...
        migrations.AlterField(
            ...
            field=models.JSONField(
                ...
                help_text=(
                    "Optional named corners: nw, ne, se, sw. "
                    "Empty linked properties default to their estate boundary."
                ),
                verbose_name="Boundary Coordinates",
            ),
        ),
