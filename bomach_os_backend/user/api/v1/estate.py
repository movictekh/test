...
def get_estate_field_choices(request):
    ...
    return {
        "estate_type": [
            {"value": c[0], "label": c[1]}
            for c in Estate.ESTATE_TYPE_CHOICES
        ],
        "estate_status": [
            {"value": c[0], "label": c[1]}
            for c in Estate.ESTATE_STATUS_CHOICES
        ],
        "area_unit": [
            {"value": c[0], "label": c[1]}
            for c in Estate.AREA_UNIT_CHOICES
        ],
    }
    ...
    try:
        ...
        estate = Estate.objects.create(
            **data,
            ...
            boundary=[{"lat": float(c.lat), "lng": float(c.lng)} for c in payload.boundary] if payload.boundary else [],
        )
...
@estate_api.put("/{estate_id}", response={200: EstateSchema, 400: MessageSchema, 404: MessageSchema})
...
def update_estate(request, estate_id: int, payload: EstateUpdateSchema):
    ...
    try:
        ...
        if 'boundary' in update_data:
            ...
            estate.boundary = [{"lat": float(c["lat"]), "lng": float(c["lng"])} for c in boundary_data] if boundary_data else []
...
@estate_api.get("/{estate_id}/properties/choices/fields", response=PropertyChoicesSchema)
def get_property_field_choices(request, estate_id: int):
    """Get available choices for property fields"""
    return {
        "property_type": [
            {"value": c[0], "label": c[1]}
            for c in Property.PROPERTY_TYPE_CHOICES
        ],
        "property_status": [
            {"value": c[0], "label": c[1]}
            for c in Property.PROPERTY_STATUS_CHOICES
        ],
        "residential_building_type": [
            {"value": c[0], "label": c[1]}
            for c in Property.RESIDENTIAL_TYPE_CHOICES
        ],
        "commercial_building_type": [
            {"value": c[0], "label": c[1]}
            for c in Property.COMMERCIAL_TYPE_CHOICES
        ],
        "area_unit": [
            {"value": c[0], "label": c[1]}
            for c in Property.AREA_UNIT_CHOICES
        ],
    }


@estate_api.get("/{estate_id}/properties", response={200: List[PropertySchema], 404: MessageSchema})
@paginate(LimitOffsetPagination, page_size=10)
...
def list_properties(
    ...
    properties = Property.objects.select_related('estate').prefetch_related('images').filter(
        estate_id=estate_id
    )
...
@estate_api.get("/{estate_id}/properties/{property_id}", response={200: PropertySchema, 404: MessageSchema})
@require_permission("properties", "view")
def get_property(request, estate_id: int, property_id: int):
    """Get property details"""
    try:
        prop = Property.objects.select_related('estate').prefetch_related('images').get(
...
@estate_api.post("/{estate_id}/properties", response={201: PropertySchema, 400: MessageSchema, 404: MessageSchema})
...
@estate_api.put("/{estate_id}/properties/{property_id}", response={200: PropertySchema, 400: MessageSchema, 404: MessageSchema})
@require_permission("properties", "update")
def update_property(request, estate_id: int, property_id: int, payload: PropertyUpdateSchema):
    """Update a property"""
    try:
        prop = Property.objects.select_related('estate').get(id=property_id, estate_id=estate_id)
        update_data = payload.dict(exclude_unset=True)

        # Handle boundary update
        if 'boundary' in update_data:
            boundary_data = update_data.pop('boundary')
            prop.boundary = [{"lat": float(c["lat"]), "lng": float(c["lng"])} for c in boundary_data] if boundary_data else []
...
@estate_api.delete("/{estate_id}/properties/{property_id}", response={200: MessageSchema, 404: MessageSchema})
...
@estate_api.get("/{estate_id}/stats", response={200: EstateStatsSchema, 404: MessageSchema})
@require_permission("estates", "view")
def estate_stats(request, estate_id: int):
    ...
    sold_value = props.filter(status='sold').aggregate(sum=models.Sum('price'))['sum'] or Decimal('0')
...
@estate_api.get("/{estate_id}/layout", response={200: List[PlotLayoutSchema], 404: MessageSchema})
@require_permission("properties", "list")
def estate_layout(request, estate_id: int):
    ...
    props = Property.objects.filter(estate_id=estate_id).order_by('plot_number', 'property_name')
    return 200, [
        {
            'id': p.id,
...
@require_permission("properties", "update")
def quick_update_plot(request, estate_id: int, property_id: int, payload: PlotQuickUpdateSchema):
...
def list_all_properties(
    ...
    properties = Property.objects.select_related(
        'estate'
    ).prefetch_related('images').filter(estate__isnull=True)
...
@estate_api.get("/properties/all/{property_id}", response={200: PropertySchema, 404: MessageSchema})
@require_permission("properties", "view")
...
@estate_api.put("/properties/all/{property_id}", response={200: PropertySchema, 400: MessageSchema, 404: MessageSchema})
@require_permission("properties", "update")
def update_standalone_property(request, property_id: int, payload: PropertyUpdateSchema):
...
@estate_api.delete("/properties/all/{property_id}", response={200: MessageSchema, 404: MessageSchema})
@require_permission("properties", "delete")
