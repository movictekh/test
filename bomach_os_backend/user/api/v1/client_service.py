    ...
    try:
        ...
        return 200, {
            ...
            "under_review": qs.filter(
                status=ServiceRequest.STATUS.UNDER_REVIEW
            ).count(),
...
@client_service_api.get(
    "/recent-orders",
    response={200: List[ServiceRequestDashboardResponseSchema], 400: MessageSchema},
)
def recent_orders(request):
    try:
        recent_requests = (
            ServiceRequest.objects.filter(client=request.user)
            .select_related("service", "client", "invoice")
...
@client_service_api.get(
    "/{id}", response={200: ClientServiceResponseSchema, 404: MessageSchema}
)
...
@service_request_api.post(
    "", response={201: ServiceRequestFullResponseSchema, 400: MessageSchema}
)
def create_request(request, payload: ServiceRequestCreateSchema):
    try:
        service = get_object_or_404(
            ClientService, id=payload.service_id, is_active=True
        )
...
@service_request_api.get(
    "/{id}", response={200: ServiceRequestFullResponseSchema, 404: MessageSchema}
)
...
def list_client_invoices(request):
    ...
    return Invoice.objects.filter(client=request.user.client_profile).prefetch_related(
        "service_requests"
    )


@service_request_api.get(
    "/payments/{invoice_id}", response=List[PaymentSubmissionResponseSchema]
)
def list_invoice_submissions(request, invoice_id: int):
    ...
    return PaymentSubmission.objects.filter(
        invoice_id=invoice_id, client=request.user.client_profile
...
@service_request_api.get(
    "/admin/service-requests", response=List[ServiceRequestDashboardResponseSchema]
)
def list_all_service_requests(request, client_id: Optional[int] = None):
    qs = ServiceRequest.objects.select_related("service", "client", "invoice")
    if client_id:
...
@service_request_api.get(
    "/admin/service-requests/{request_id}", response=ServiceRequestFullResponseSchema
)
