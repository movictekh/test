...
@cases_api.get("/", response={200: List[LegalCaseResponse], 400: ErrorResponse}, auth=JWTAuthenticator())
...
def list_legal_cases(
    ...
    try:
        ...
        if start_date:
            cases = cases.filter(date_filed__gte=datetime.fromisoformat(start_date).date())

        if end_date:
            cases = cases.filter(date_filed__lte=datetime.fromisoformat(end_date).date())
        ...
        if has_upcoming_hearing is not None:
            if has_upcoming_hearing:
                ...
                cases = cases.filter(Q(next_hearing__isnull=True) | Q(next_hearing__lt=date.today()))
...
@cases_api.post("/", response={201: LegalCaseResponse, 400: ErrorResponse}, auth=JWTAuthenticator())
@require_permission("legal_cases", "create")
def create_legal_case(request: HttpRequest, payload: CreateLegalCaseRequest):
    try:
        # Validate case type
        valid_case_types = [
            'contract_dispute', 'employment', 'regulatory', 'civil',
            'criminal', 'corporate', 'intellectual_property'
        ]
        if payload.case_type not in valid_case_types:
            return 400, {"detail": f"Invalid case type. Must be one of: {', '.join(valid_case_types)}"}

        # Validate status
        valid_statuses = [
            'in_progress', 'settled', 'open', 'closed',
            'pending', 'dismissed', 'won', 'lost'
        ]
...
@cases_api.get("/statistics/overview/", response={200: LegalCaseStatisticsResponse, 400: ErrorResponse}, auth=JWTAuthenticator())
def get_case_statistics(request: HttpRequest):
    try:
        ...
        total_exposure = LegalCase.objects.aggregate(
            total=Sum('exposure')
        )['total'] or Decimal('0.00')

        # Count upcoming hearings
        today = date.today()
        upcoming_hearings = LegalCase.objects.filter(
            next_hearing__gte=today
        ).count()

        # Count by case type
        case_type_counts = LegalCase.objects.values('case_type').annotate(count=Count('id'))
...
@cases_api.get("/upcoming-hearings/", response={200: List[LegalCaseResponse], 400: ErrorResponse}, auth=JWTAuthenticator())
...
@cases_api.get("/{case_id}/", response={200: LegalCaseResponse, 404: ErrorResponse}, auth=JWTAuthenticator())
...
@cases_api.put("/{case_id}/", response={200: LegalCaseResponse, 400: ErrorResponse, 404: ErrorResponse}, auth=JWTAuthenticator())
@require_permission("legal_cases", "update")
def update_legal_case(request: HttpRequest, case_id: int, payload: UpdateLegalCaseRequest):
    try:
        case = LegalCase.objects.get(id=case_id)
...
@cases_api.patch("/{case_id}/status/", response={200: LegalCaseResponse, 400: ErrorResponse, 404: ErrorResponse}, auth=JWTAuthenticator())
@require_permission("legal_cases", "update_status")
def update_case_status(request: HttpRequest, case_id: int, payload: UpdateCaseStatusRequest):
    try:
        case = LegalCase.objects.get(id=case_id)
...
@cases_api.delete("/{case_id}/", response={200: MessageSchema, 404: ErrorResponse}, auth=JWTAuthenticator())
