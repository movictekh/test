...
def _quote_items(approval_status: str = 'pending') -> List[Dict[str, Any]]:
    if approval_status == 'pending':
        qs = Quote.objects.select_related('service', 'created_by', 'required_approver_role').filter(status='awaiting_approval')
    elif approval_status == 'approved':
        qs = Quote.objects.select_related('service', 'created_by', 'required_approver_role').filter(status__in=['sent', 'accepted'])
    elif approval_status == 'rejected':
        qs = Quote.objects.select_related('service', 'created_by', 'required_approver_role').filter(status='rejected')
    else:
        return []

    items = []
    for q in qs:
        subject = f"{q.service.name} quotation" if q.service else f"Quotation {q.quote_number}"
        items.append({
            'id': f"quotation-{q.id}",
            'source': 'quotation',
            ...
            'approver_name': q.required_approver_role.name if q.required_approver_role else None,
            'amount': q.amount,
            'created_at': q.created_at,
            'status': approval_status,
            'action_label': 'Approve & Send' if approval_status == 'pending' else '—',
            'approve_url': f"/api/v1/quotes/{q.id}/approve" if approval_status == 'pending' else None,
            'reject_url': None,
        })
    return items
...
def _deliverable_items(approval_status: str = 'pending') -> List[Dict[str, Any]]:
    if approval_status == 'pending':
        ...
        qs = ServiceDeliverable.objects.select_related('order', 'created_by').filter(status='approved')
    elif approval_status == 'rejected':
        qs = ServiceDeliverable.objects.select_related('order', 'created_by').filter(status='rejected')
    else:
        return []
    ...
    for d in qs:
        ...
        items.append({
            ...
            'approve_url': f"/api/v1/orders/{d.order_id}/deliverables/{d.id}/approve" if approval_status == 'pending' else None,
            'reject_url': f"/api/v1/orders/{d.order_id}/deliverables/{d.id}/reject" if approval_status == 'pending' else None,
        })
    return items
...
def _expense_items(approval_status: str = 'pending') -> List[Dict[str, Any]]:
    ...
    for e in qs:
        items.append({
            ...
            'status': approval_status,
            'action_label': 'Approve' if approval_status == 'pending' else '—',
            'approve_url': f"/api/v1/expenses/{e.id}/approve" if approval_status == 'pending' else None,
            'reject_url': f"/api/v1/expenses/{e.id}/reject" if approval_status == 'pending' else None,
        })
    return items


def _apply_search(items: List[Dict[str, Any]], search: Optional[str]) -> List[Dict[str, Any]]:
    if not search:
        return items
    needle = search.lower()
    return [
...
def _resolved_records(window_start) -> List[Any]:
    ...
    records.extend(
        ...
        for q in Quote.objects.filter(status__in=['sent', 'accepted'], approved_at__gte=window_start)
    )
    records.extend(
        (q.created_at, q.client_responded_at)
        for q in Quote.objects.filter(status='rejected', client_responded_at__gte=window_start)
    )
    records.extend(
        (d.created_at, d.approved_at)
        for d in ServiceDeliverable.objects.filter(status='approved', approved_at__gte=window_start)
    )
    records.extend(
        (d.created_at, d.rejected_at)
        for d in ServiceDeliverable.objects.filter(status='rejected', rejected_at__gte=window_start)
    )
    records.extend(
...
def approval_queue_stats(
    ...
    high_value_count = len(
        [it for it in pending_items if it['amount'] is not None and it['amount'] > threshold]
    )
    ...
    within_target = sum(
        1 for created_at, resolved_at in resolved
        if resolved_at and (resolved_at - created_at).total_seconds() <= target_seconds
    )

    if total_resolved > 0:
        sla_percent = (Decimal(within_target) / Decimal(total_resolved) * Decimal('100')).quantize(Decimal('0.01'))
    else:
...
def list_approval_queue(
    ...
    if high_value:
        threshold = high_value_threshold or DEFAULT_HIGH_VALUE_THRESHOLD
        items = [it for it in items if it['amount'] is not None and it['amount'] > threshold]
