...
from user.api.schemas.audit import (
    AuditResponse,
    CreateAuditRequest,
    UpdateAuditRequest,
    UpdateAuditStatusRequest,
    UpdateAuditScoreRequest,
    AuditStatisticsResponse,
    AuditPerformanceTrendsResponse,
...
def list_audits(
    ...
    try:
        ...
        if start_date:
            audits = audits.filter(audit_date__gte=datetime.fromisoformat(start_date).date())

        if end_date:
            audits = audits.filter(audit_date__lte=datetime.fromisoformat(end_date).date())
...
@audit_api.get("/statistics/overview/", response={200: AuditStatisticsResponse, 400: ErrorResponse})
def get_audit_statistics(request: HttpRequest):
    try:
        ...
        # Count by audit type
        audit_type_counts = Audit.objects.values('audit_type').annotate(count=Count('id'))
        audits_by_type = {item['audit_type']: item['count'] for item in audit_type_counts}
        ...
        audits_by_score_category = {
            ...
            "Needs Improvement": scored_audits.filter(score__gte=60, score__lt=70).count(),
...
@audit_api.get("/statistics/performance-trends/", response={200: AuditPerformanceTrendsResponse, 400: ErrorResponse})
def get_performance_trends(request: HttpRequest, period: str = "month", months: int = 12):
    try:
        ...
        trends_data = audits.annotate(
            period_date=trunc_func('audit_date')
        ).values('period_date').annotate(
            total_audits=Count('id'),
            average_score=Avg('score'),
            passed=Count(Case(When(score__gte=70, then=1), output_field=IntegerField()))
        ).order_by('period_date')

        # Format trends
        trends = []
        for item in trends_data:
            pass_rate = (item['passed'] / item['total_audits'] * 100) if item['total_audits'] > 0 else 0
            trends.append({
                "period": item['period_date'].strftime('%Y-%m-%d') if item['period_date'] else 'N/A',
                "average_score": float(item['average_score']) if item['average_score'] else 0,
        ...
        return 200, {
            "period": period,
            "trends": trends,
            "overall_improvement": float(overall_improvement) if overall_improvement is not None else None
        }
    except Exception as e:
        return 400, {"detail": str(e)}


@audit_api.get("/failing-audits/", response={200: List[AuditResponse], 400: ErrorResponse})
@paginate(LimitOffsetPagination, page_size=10)
...
@audit_api.get("/auditors/", response={200: List[AuditorSummaryResponse], 400: ErrorResponse})
@paginate(LimitOffsetPagination, page_size=10)
def list_auditors(request: HttpRequest):
    try:
        # Group audits by auditor
        auditor_data = Audit.objects.values('auditor_name').annotate(
            total_audits=Count('id'),
            average_score=Avg('score')
...
def create_audit(request: HttpRequest, payload: CreateAuditRequest):
    try:
        # Validate audit type
        valid_audit_types = [
            'external', 'regulatory', 'internal', 'customer',
            'financial', 'operational', 'compliance'
        ]
        if payload.audit_type not in valid_audit_types:
            return 400, {"detail": f"Invalid audit type. Must be one of: {', '.join(valid_audit_types)}"}

        # Validate status
        valid_statuses = [
            'completed', 'in_progress', 'scheduled', 'draft', 'cancelled', 'failed'
        ]
...
@audit_api.put("/{audit_id}/", response={200: AuditResponse, 400: ErrorResponse, 404: ErrorResponse})
...
@audit_api.patch("/{audit_id}/status/", response={200: AuditResponse, 400: ErrorResponse, 404: ErrorResponse})
@require_permission("compliance_audits", "update_status")
def update_audit_status(request: HttpRequest, audit_id: int, payload: UpdateAuditStatusRequest):
    try:
        audit = Audit.objects.get(id=audit_id)
...
@audit_api.patch("/{audit_id}/score/", response={200: AuditResponse, 400: ErrorResponse, 404: ErrorResponse})
@require_permission("compliance_audits", "update_score")
def update_audit_score(request: HttpRequest, audit_id: int, payload: UpdateAuditScoreRequest):
    try:
        audit = Audit.objects.get(id=audit_id)
