...
def calculate_rank(employee):
    ...
    rank = next(i for i, item in enumerate(member_scores) if item['id'] == employee.id) + 1
...
def get_dashboard_summary(request):
    ...
    avg_rating = employee.reports.filter(
        status='approved',
        created_at__date__range=[start_date, today] 
...
def get_performance_card(request):
    ...
    total_days, on_time_days, late_days, absent_days, punc_score = calculate_punctuality(employee)
    
    monthly_reports = employee.reports.filter(created_at__date__range=[start_date, today])
