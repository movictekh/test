...
class Event(BaseModel):
    ...
    event_time_from = models.DateTimeField(
        default=timezone.now, help_text="Start time of the event (UTC)"
    )

    event_time_to = models.DateTimeField(
        default=get_default_event_end_time, help_text="End time of the event (UTC)"
    )
    ...
    organizer = models.ForeignKey(
        User, on_delete=models.SET_NULL, null=True, verbose_name="Event Organizer"
    )
    ...
    def clean(self):
        ...
        if self.event_time_to <= self.event_time_from:
            raise ValidationError(
                {"event_time_to": "Event end time must be after start time."}
            )
        if self.reminder_offset is not None and self.reminder_offset < 0:
            raise ValidationError(
                {"reminder_offset": "Reminder offset must be a non-negative integer."}
            )
        if self.event_time_from < timezone.now():
            raise ValidationError(
                {"event_time_from": "Event start time cannot be in the past."}
            )
