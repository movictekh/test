...
class LegalCase(BaseModel):
    ...
    case_type = models.CharField(
        max_length=50, choices=CASE_TYPE_CHOICES, help_text="Type of legal case"
    )
    ...
    next_hearing = models.DateField(
        null=True, blank=True, help_text="Date of the next scheduled hearing"
    )
    ...
    date_filed = models.DateField(
        null=True, blank=True, help_text="Date when the case was filed"
    )

    date_closed = models.DateField(
        null=True, blank=True, help_text="Date when the case was closed/settled"
    )
    ...
    def clean(self):
        ...
        if self.case_type and (self.case_type not in valid_types):
            raise ValidationError(
                {
                    "case_type": f"Invalid case type. Must be one of: {', '.join(valid_types)}"
                }
            )
        ...
        if self.date_filed and self.date_closed:
            if self.date_closed < self.date_filed:
                raise ValidationError(
                    {"date_closed": "Date closed cannot be before date filed."}
                )
