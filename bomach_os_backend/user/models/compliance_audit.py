...
class Audit(BaseModel):
    ...
    audit_type = models.CharField(
        max_length=50, choices=AUDIT_TYPE_CHOICES, help_text="Type of audit"
    )

    auditor_name = models.CharField(
        max_length=200, help_text="Name of the auditor (e.g., 'Mrs. Funmi Adebayo')"
    )
    ...
    findings = models.TextField(blank=True, help_text="Key findings from the audit")
    ...
    start_date = models.DateField(
        null=True, blank=True, help_text="Date when audit started"
    )

    end_date = models.DateField(
        null=True, blank=True, help_text="Date when audit was completed"
    )

    auditor_organization = models.CharField(
        max_length=300, blank=True, help_text="Organization the auditor represents"
    )

    department = models.CharField(
        max_length=200, blank=True, help_text="Department being audited"
    )
