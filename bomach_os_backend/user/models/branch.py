...
class Branch(BaseModel):
    ...
    def clean(self):
        ...
        if self.operational_status and self.operational_status not in valid_statuses:
            raise ValidationError({'operational_status': f"Invalid status. Must be one of: {', '.join(valid_statuses)}"})
        valid_roles = [choice[0] for choice in self.BRANCH_ROLE_CHOICES]
        if self.branch_role and self.branch_role not in valid_roles:
            raise ValidationError({'branch_role': f"Invalid branch role. Must be one of: {', '.join(valid_roles)}"})
    ...
    def clean(self):
        ...
        if self.is_open:
            if not self.open_time or not self.close_time:
                raise ValidationError("Open and close times are required when the branch is open.")
