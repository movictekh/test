...
class Attendance(BaseModel):
    ...
    timestamp = models.DateTimeField(
        default=timezone.now, help_text="When the clock-in/out occurred"
    )
    ...
    location_verified = models.BooleanField(
        default=False, help_text="Whether the location was verified against whitelist"
    )
    ...
    def clean(self):
        ...
        if self.attendance_type and (self.attendance_type not in valid_types):
            raise ValidationError(
                {
                    "attendance_type": f"Invalid attendance type. Must be one of: {', '.join(valid_types)}"
                }
            )
        ...
        if self.verification_method and (self.verification_method not in valid_methods):
            raise ValidationError(
                {
                    "verification_method": f"Invalid verification method. Must be one of: {', '.join(valid_methods)}"
                }
            )
        if self.match_confidence is not None:
            if self.match_confidence < Decimal("0") or self.match_confidence > Decimal(
                "100"
            ):
                raise ValidationError(
                    {"match_confidence": "Match confidence must be between 0 and 100."}
                )
