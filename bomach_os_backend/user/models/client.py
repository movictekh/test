...
class Lead(BaseModel):
    ...
    def clean(self):
        ...
        if self.gender and (self.gender not in valid_genders):
            raise ValidationError({'gender': f"Invalid gender. Must be one of: {', '.join(valid_genders)}"})
        ...
        if self.marital_status and (self.marital_status not in valid_statuses):
            raise ValidationError({'marital_status': f"Invalid marital status. Must be one of: {', '.join(valid_statuses)}"})
        valid_sources = [choice[0] for choice in self.SourceChoices.choices]
        if self.source and (self.source not in valid_sources):
            raise ValidationError({'source': f"Invalid source. Must be one of: {', '.join(valid_sources)}"})
        ...
        if self.status and (self.status not in valid_lead_statuses):
            raise ValidationError({'status': f"Invalid status. Must be one of: {', '.join(valid_lead_statuses)}"})
