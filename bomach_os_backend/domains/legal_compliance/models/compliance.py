...
class ComplianceRecord(BaseModel):
    ...
    email_address = models.EmailField(
        validators=[EmailValidator()], help_text="Email address of the user"
    )
    ...
    compliance_type = models.CharField(
        max_length=100, choices=COMPLIANCE_TYPE_CHOICES, help_text="Type of compliance"
    )
    ...
    issuing_authority = models.CharField(
        max_length=255, help_text="Authority that issued the compliance"
    )
    ...
    description = models.TextField(
        blank=True, null=True, help_text="Description of the compliance"
    )
    ...
    contact_email = models.EmailField(
        blank=True, null=True, validators=[EmailValidator()], help_text="Contact email"
    )
    ...
    def clean(self):
        ...
        if self.compliance_type and (self.compliance_type not in valid_types):
            raise ValidationError(
                {
                    "compliance_type": f"Invalid compliance type. Must be one of: {', '.join(valid_types)}"
                }
            )
        ...
        if self.priority_level and (self.priority_level not in valid_priorities):
            raise ValidationError(
                {
                    "priority_level": f"Invalid priority. Must be one of: {', '.join(valid_priorities)}"
                }
            )
        if self.date_of_issue and self.expiry_date:
            if self.expiry_date <= self.date_of_issue:
                raise ValidationError(
                    {"expiry_date": "Expiry date must be after date of issue."}
                )
