...
def get_kpis(
    ...
    conversion = (
        (Decimal(converted_quotes) / Decimal(total_quotes) * 100).quantize(Decimal('0.01'))
        if total_quotes else Decimal('0.00')
    )

    # --- Average response time (ServiceRequest → first activity) ---
    sr_filter = _date_filters(date_from, date_to)
    requests_with_activity = ServiceRequest.objects.filter(
        sr_filter,
    ...
    if total_revenue > 0:
        margin = ((total_revenue - total_expenses) / total_revenue * 100).quantize(Decimal('0.01'))
    else:
        margin = Decimal('0.00')
    ...
    on_time_pct = (
        ...
        if total_completed else Decimal('0.00')
    )
...
def get_service_performance(
    ...
    for row in orders:
        ...
        results.append(ServicePerformanceItem(
            service_name=name,
...
def export_service_performance(
    ...
    return HttpResponse(
        ...
        headers={'Content-Disposition': 'attachment; filename="service-performance.csv"'},
    )
...
def get_branch_performance(
    ...
    branches = ServiceRequest.objects.filter(
        sr_filter,
    ...
    for row in branches:
        ...
        csat = (csat_avg * 20).quantize(Decimal('0.01')) if csat_avg else Decimal('0.00')

        results.append(BranchPerformanceItem(
