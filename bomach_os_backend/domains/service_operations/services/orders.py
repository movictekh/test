...
def log_order_activity(order, activity_type, note, *, created_by, visibility="internal_client", progress=None, next_action=""):
    return ServiceOrderActivity.objects.create(
        order=order, activity_type=activity_type, visibility=visibility,
        note=note, progress=progress, next_action=next_action, created_by=created_by,
    )
...
def update_order(order, payload, *, user):
    ...
    if "invoice_id" in data and data["invoice_id"] != order.invoice_id:
        raise ValidationError("Invoice links cannot be changed from the order update endpoint.")
    ...
    with transaction.atomic():
        ...
        order.save()
        log_order_activity(
            order, "control_update",
            f"Service order updated. Status: {order.order_status}; progress: {order.progress}%.",
            progress=order.progress, next_action=order.next_action, created_by=user,
        )
...
def create_order_activity(order, payload, *, user):
    ...
    with transaction.atomic():
        ...
        return log_order_activity(
            order, payload.activity_type, payload.note,
            visibility=payload.visibility, progress=data.get("progress"),
            next_action=data.get("next_action") or "", created_by=user,
        )
...
def create_order_milestone(order, payload, *, user):
    ...
    with transaction.atomic():
        ...
        log_order_activity(
            order, "milestone_added", f"Milestone added: {milestone.name}.",
...
def complete_order_milestone(order, milestone_id, *, user):
    ...
    with transaction.atomic():
        ...
        next_milestone = order.milestones.filter(status="pending", sort_order__gte=milestone.sort_order).order_by("sort_order", "id").first()
        if next_milestone:
        ...
        log_order_activity(
            order, "stage_advanced", f"Milestone completed: {milestone.name}.",
            visibility="internal_client" if milestone.client_visible else "internal",
            progress=order.progress, next_action=order.next_action, created_by=user,
        )
    return order
...
def reopen_order_milestone(order, milestone_id, *, user):
    ...
    with transaction.atomic():
        ...
        order.refresh_progress_from_milestones()
        log_order_activity(
            order, "milestone_reopened", f"Milestone reopened: {milestone.name}.",
            visibility="internal_client" if milestone.client_visible else "internal",
            progress=order.progress, next_action=order.next_action, created_by=user,
        )
    return order
...
def create_order_task(order, payload, *, user):
    ...
    with transaction.atomic():
        ...
        log_order_activity(order, "task_created", f"Execution task {task.task_number} created: {task.title}.", created_by=user, next_action=order.next_action)
    return task
...
def update_order_task(order, task, payload, *, user):
    ...
    with transaction.atomic():
        ...
        if "assignee_ids" in supplied:
            task.assignees.set(assignee_ids)
        log_order_activity(order, "task_updated", f"Execution task {task.task_number} updated. Status: {task.status}.", created_by=user, next_action=order.next_action)
    return task
...
def advance_order_task(order, task, *, user):
    ...
    with transaction.atomic():
        ...
        task.save()
        log_order_activity(order, "task_advanced", f"Execution task {task.task_number} advanced to {task.status}.", created_by=user, next_action=order.next_action)
    return task
...
def delete_order_task(order, task, *, user):
    ...
    with transaction.atomic():
        task.delete()
        log_order_activity(order, "task_updated", f"Execution task {number} deleted.", created_by=user, visibility="internal")
...
def create_order_deliverable(order, payload, *, user):
    ...
    with transaction.atomic():
        ...
        log_order_activity(order, "deliverable_added", f"Deliverable {obj.deliverable_number} added: {obj.title}.", created_by=user, visibility="internal_client" if obj.client_visible else "internal", next_action=order.next_action)
    return obj
...
def update_order_deliverable(order, obj, payload, *, user):
    ...
    with transaction.atomic():
        ...
        obj.save()
        log_order_activity(order, "deliverable_added", f"Deliverable {obj.deliverable_number} updated. Status: {obj.status}.", created_by=user, visibility="internal_client" if obj.client_visible else "internal", next_action=order.next_action)
    return obj
...
def approve_order_deliverable(order, obj, *, user, client_mode=False):
    ...
    with transaction.atomic():
        ...
        obj.save()
        note = f"Client approved deliverable {obj.deliverable_number}." if client_mode else f"Deliverable {obj.deliverable_number} approved."
        log_order_activity(order, "deliverable_approved", note, created_by=user, visibility="internal_client" if obj.client_visible else "internal", next_action="" if client_mode else order.next_action)
    return obj
...
def reject_order_deliverable(order, obj, *, reason, user, client_mode=False):
    ...
    with transaction.atomic():
        ...
        obj.save()
        note = f"Client rejected deliverable {obj.deliverable_number}." if client_mode else f"Deliverable {obj.deliverable_number} rejected."
        log_order_activity(order, "deliverable_rejected", note, created_by=user, visibility="internal_client" if obj.client_visible else "internal", next_action="" if client_mode else order.next_action)
    return obj
...
def delete_order_deliverable(order, obj, *, user):
    ...
    with transaction.atomic():
        obj.delete()
        log_order_activity(order, "deliverable_added", f"Deliverable {number} deleted.", created_by=user, visibility="internal")
