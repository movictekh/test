...
def _create_answer_rows(obj):
    rows = [
        ServiceRequestAnswer(
            ...
            value=obj.answers_snapshot.get(f["key"]),
            sort_order=f.get("sort_order", 0),
        )
        for f in obj.form_snapshot.get("fields", [])
    ]
...
def create_service_request(
    ...
    service = get_object_or_404(Service, id=data.pop("service_id"))
    sub = None
    if rel["subservice_id"]:
        sub = get_object_or_404(
            ServiceSubService, id=rel["subservice_id"], service=service
        )
    with transaction.atomic():
        obj = ServiceRequest.objects.create(
            ...
            **{k: v for k, v in rel.items() if k != "subservice_id"},
            **data,
        )
...
def update_staff_request(obj, payload, *, updated_by):
    ...
    for p, a in {
        ...
        if p in data:
            setattr(obj, a, data.pop(p))
    for a, v in data.items():
        setattr(obj, a, v)
    obj.save()
    log_activity(
        obj,
        "status_change" if old != obj.status else "control_update",
...
def create_request_activity(obj, payload, *, created_by):
    ...
    ensure_choice(payload.outcome, ServiceRequestActivity.OUTCOME_CHOICES, "outcome")
    return log_activity(
        obj,
        payload.activity_type,
        payload.note,
...
def create_request_attachment(obj, payload, *, uploaded_by):
    x = ServiceRequestAttachment(request=obj, uploaded_by=uploaded_by, **payload.dict())
    x.full_clean()
    x.save()
    return x
