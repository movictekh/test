...
describe('property batch builder', () => {
    ...
    const rows = buildPropertyBatch(
      {
        ...
        price: 80000000,
        status: 'available',
        buildingTypeResidential: 'duplex',
        bedrooms: 4,
        bathrooms: 4,
        totalAreaResidential: 300,
      },
      2,
      1,
      'Residence',
  ...
  it('creates commercial batches without plot-only fields', () => {
    const rows = buildPropertyBatch(
      {
        ...
        price: 150000000,
        status: 'available',
        buildingTypeCommercial: 'office',
        totalAreaCommercial: 500,
        numberOfFloors: 3,
        unitsOffices: 12,
      },
      2,
    ...
    expect(rows.map((x) => x.input.propertyName)).toEqual([
      'Commercial Unit 5',
      'Commercial Unit 6',
    ])
    expect(rows.every((x) => x.input.plotNumber === undefined)).toBe(true)
